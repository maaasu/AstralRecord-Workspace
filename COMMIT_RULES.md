...
`60_tool/generate-status-types.ps1`と`60_tool/generate-tag-types.ps1`が共有カタログから生成するKotlin/Java/C#/TypeScriptソースは、プラットフォーム間の型同期を保証するための管理対象ソースとして例外的にコミットする。生成物を直接編集せず、YAMLと生成物を同じcommitへ含める。
