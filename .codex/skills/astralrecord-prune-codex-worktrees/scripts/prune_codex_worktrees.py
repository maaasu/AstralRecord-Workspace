...
def management_action(category: str) -> str:
    actions = {
        "REMOVABLE_WORKTREE": "develop 取り込み済みで clean。明示的な execute cleanup で削除対象。",
        "REMOVABLE_BRANCH": "develop 取り込み済みで worktree 未接続。明示的な execute cleanup で branch 削除対象。",
        "STALE_METADATA": "path が存在しない worktree metadata。execute cleanup で git worktree prune 対象。",
        "UNREGISTERED_PATH": "Git checkout らしいが git worktree 未登録。削除前に手動確認する。",
        "NON_GIT_DIRECTORY": "git worktree ではないディレクトリ。削除前に手動確認する。",
        "DIRTY_WORKTREE": "develop 取り込み済みだが未コミット差分が残っている。内容を確認して保存・破棄・別 commit を判断する。",
        "UNMERGED_WORKTREE": "branch 先端が develop 未取り込み。個別 finalize / rebase するか破棄判断する。",
        "UNMERGED_BRANCH": "develop 未取り込みで登録 worktree がない branch。worktree 再作成・個別 finalize・破棄を判断する。",
        "DETACHED_WORKTREE": "detached HEAD の worktree。競合・rebase 中断・手動 checkout 状態を確認する。",
        "DETACHED_HEAD_BRANCH": "merged branch の先端を detached worktree が保持中。branch 削除前に detached worktree を確認する。",
        "NON_CODEX_WORKTREE": "codex/* 以外の worktree。この掃除対象から外す。",
    }
    return actions.get(category, "手動確認する。")
...
def preserved_manual_notes(path: Path) -> str:
    if not path.exists():
        return "- 意図的に保持する worktree の判断理由をここに追記する。\n"
    ...
    if notes == "- Add manual decisions here when a worktree is intentionally kept.":
        return "- 意図的に保持する worktree の判断理由をここに追記する。\n"
    return f"{notes}\n" if notes else "- 意図的に保持する worktree の判断理由をここに追記する。\n"
...
def write_management_report(
    ...
    lines = [
        "# AstralRecord Worktree 管理",
        "",
        "このファイルは `$astralrecord-prune-codex-worktrees` が生成するローカル管理スナップショットです。",
        "残っている worktree が cleanup 漏れなのか、作業中・停止中・dirty・detached なのかを区別するために使います。",
        "",
        "## スナップショット",
        "",
        f"- 生成日時: {now}",
        ...
        "## 状態一覧",
        "",
        "| 分類 | branch/path | worktree/path | 判断 |",
        "|:--|:--|:--|:--|",
    ]
    ...
    for category in ordered_categories:
        ...
        for value in values:
            ...
            wrote_row = True
    if not wrote_row:
        lines.append("| `CLEAN` | `none` | `none` | cleanup 対象や確認項目はありません。 |")
