...
6. Verify:
   ...
   - For feature/behavior changes, executable scripts, schemas/data contracts, workspace skill logic, multi-file work, or security/concurrency/data-integrity changes, capture complete verification output including standard error and inspect warnings as well as the exit status. Resolve warnings caused by the task and rerun the same check. For a remaining warning, classify it as pre-existing (verify against current local `develop` when practical) or external/toolchain-originated, and report the command, warning summary, classification, and reason. Do not hand off a change with a new unexplained warning or a task-originated warning unless the user explicitly approved its deferral.
