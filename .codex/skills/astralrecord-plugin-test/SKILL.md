...
- 恒久テストは、コンテンツ非依存で複数機能から再利用される共通ロジックであり、純粋な計算・入力正規化・共通の状態遷移・不変条件・冪等性・補償・データ保全・権限境界のいずれかを守るものに限る。疑似サーバーを起動せず決定的に検証でき、不具合時の影響が大きい、または手作業で見逃しやすいことも必須とする。
...
- MockBukkit等の疑似サーバーを利用する検証は、`AdHoc<目的>Test` または `<目的>OneShotTest` の一時診断に限る。テストsource、専用support source、ビルド依存は結果確認後にすべて削除し、コミットしない。恒久化する契約はBukkitアダプターから純ロジックを分離してJUnitで検証する。
- 通常のプラグインJUnitと一時的な疑似サーバーテストでは、本番 `<task-root>\40_filebase` を読み込まず、参照せず、接続しない。マスタ形状が必要なら、テスト内の最小固定テスト用データ、インラインのデータ、またはテストダブルを使う。本番ファイルから期待値を組み立てない。
...
3. 検証層を決める
   - 適格性を満す純ロジック確認: `JUnit`
   - Bukkit境界を分離できない一度限りの再現: 一時テストとしての `MockBukkit`
...
4. 配置して実装する
   ...
   - MockBukkit用の補助クラスや依存関係は一時テストと同じ作業差分でだけ追加し、実行後に一時テストとともに削除する。`support` 配下への移動や通常名への変更で残さない
...
設計契約ではない一度限りの診断は、ファイルとクラスを `AdHoc<目的>Test`（既定）または `<目的>OneShotTest` として追加し、恒久テストへ残さない。MockBukkit等の疑似サーバーはこの一時診断でだけ使用できる。通常機能名と衝突し得る曖昧な接頭辞は一時診断の識別子にしない。次の順序を守る。

1. `AdHoc<目的>Test` または `<目的>OneShotTest` を追加する。疑似サーバーが必要なら専用support sourceとPOM依存も一時変更として追加する
...
4. 追加した一時テストを削除する。疑似サーバーを利用した場合は専用support sourceとPOM依存も削除する
...
- 一時MockBukkit診断で無理に `AstralRecord` 本体をロードしない。`ProtocolLib` や外部依存に引っかかるなら、対象クラスを隔離するか一時開発サーバーへ切り替える。
- 純ロジック -> 一時MockBukkit診断 -> 開発サーバー -> 実サーバーの複製の順で狭く確認し、MockBukkit関連のsource・support・依存はコミット前に除去する。
...
```text
$astralrecord-plugin-test を使って、<task-root>\10_plugin\AstralRecord のインベントリの二重消費防止契約が恒久テスト適格性を満たすか判定し、疑似サーバー不要なら最小のJUnit恒久テストを追加してください。Bukkit境界の確認が必要なら一時MockBukkitテストを実行後に削除してください。
```
...
## 残課題
- なし / <恒久JUnitでは扱えず、一時診断または実サーバ確認が必要な点>
```
