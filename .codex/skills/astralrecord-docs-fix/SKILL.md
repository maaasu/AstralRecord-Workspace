...
設計書だけを修正する。ソースコード、実装ファイル、データベーススキーマファイル、生成素材、実行時出力、配備ファイルは開いたり編集したりしない。設計書に書かれた実装パスは範囲を示すラベルとしてだけ扱う。
...
- 設計書対象パスと、レビュー結果のパスまたは貼り付けたレビュー本文。
...
1. 設計書対象パスとレビュー元を特定する。
   - `git rev-parse --show-toplevel` で `<task-root>` を解決する。
   - 設計書またはレビュー記録を編集する前に、`develop` ではない専用作業ブランチ/作業ツリーを必須とする。ない場合は `develop` に書き込まず、統合作業ツリー手順を使う。
...
2. `astralrecord-docs-review` の報告形式でレビュー結果を解析する。
   - `AR-DOC-*` 指摘 IDs.
   - `対象`, `関連箇所`, `修正方針`, `修正対象候補`, `修正可否`, `確信度`、`修正状態`.
...
5. ローカルの構成、見出し、用語、Wiki リンク形式、テーブル形式を保ちながら、指摘を解消する最小限の Markdown/設計変更を適用する。
...
```
   ...
   - 元の作成日時、対象パス、`docs-review` スキル名を保持する。
...
この監査を形式確認として使う。スクリプトが無関係な既存問題を報告した場合は、編集範囲を広げず別項目として列挙する。
9. 変更した設計書が `00_docs/10_Plugin設計書` 配下にある場合は、このスキルがプラグインソースを開いたり解釈したりしない場合でも、`<task-root>` から次のブラックボックス整合性ゲートも実行する。設計書のみの見出し/パス編集後に既存の恒久テスト参照が解決できるかを確認する。

```powershell
python <task-root>\.codex\skills\astralrecord-plugin-test\scripts\validate_test_traceability.py --repo-root <task-root>
```

`mvn verify` で代替しない。そのプラグイン Shade 出力はメインワークスペースの配布先を対象とする。

## 編集時の制約

- 変更は、指摘が明示的に別の場所を指していない限り、依頼された対象の設計書に限定する。
- 機能設計書ですでに使われている日本語の用語を維持する。
- 複数ファイルへ説明を重複させず、正本の設計書を修正することを優先する。
- レビュー結果またはユーザー指示が未決の設計判断を記録するよう求めている場合は、判断せずカテゴリ `9` に残す。受け入れ済みだが未実装の仕様はカテゴリ `8` に残し、2つの状態を混在させない。
- 設計書またはユーザーが示していない実装詳細を追加しない。
- レビューで確認が必要とされた挙動について、1つを選んで曖昧さを取り除かない。
- `未確認/質問`（`Q-DOC-*`）は、レビュー結果、ユーザーの指定、または許可された設計書文脈から明確に確認できる場合だけ解決する。それ以外は未解決のまま報告に残す。
...
- validate_test_traceability.py: 実行 / 未実行（理由: プラグイン設計書以外）
- ソースコード参照: していません
```
