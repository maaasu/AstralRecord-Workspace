...
- 恒久テストは、採用済み設計契約であることに加え、コンテンツ非依存の共通ロジックであり、決定的に検証でき、不具合時の影響が大きいものに限る。対象は、純粋な計算、入力正規化、共通の状態遷移・不変条件、冪等性、補償、データ保全、権限境界とする。
...
- 個別コンテンツの切り分けにテストが必要な場合は一時テストとして追加し、確認後に削除する。見た目・操作感・演出は実サーバーまたは実クライアント、マスタ内容は専用validatorで確認する。
- MockBukkitは恒久テストの既定手段としない。データ保全、権限、不可逆な状態遷移など、Bukkit境界の確認を分離できない共通契約に限り使用する。それ以外はBukkit adapterを薄くし、抽出した純ロジックをJUnitで検証する。
