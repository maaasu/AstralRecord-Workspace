...
public sealed class SkillTreeOperationRepository(AstralRecordDbContext dbContext, INetworkRuntimeService networkRuntimeService) : ISkillTreeOperationRepository
{
    ...
    public async Task<SkillTreeEditorResponse?> RegisterPlayerViewAsync(string serverId, Guid accountId, SkillTreePlayerViewRegistrationRequest request)
    {
        ...
        await UpsertViewAsync(runtime, accountId, request.PlayerStateVersion, request.EvaluationFingerprint, request.EditEligible, request.OfflineConfirmed, request.View, DateTime.UtcNow);
    ...
    public async Task<SkillTreeOperationResponse?> CreateAsync(Guid accountId, SkillTreeOperationCreateRequest request)
    {
        ...
        var reason = runtime is null || view is null ? "サーバーの更新待ちです。最新の状態を再取得してください。" : !online && !view.OfflineConfirmed ? "接続状態を確認できません。ログアウト完了後に再取得してください。" : state?.DefinitionGenerationId is null ? "既存のスキルツリー状態は移行確認が必要です。" : state.DefinitionGenerationId != request.ExpectedDefinitionGenerationId || view.DefinitionGenerationId != request.ExpectedDefinitionGenerationId || view.PlayerStateVersion != request.ExpectedPlayerStateVersion ? "最新の状態を再取得してください。" : online && (!string.Equals(presence!.ServerId, runtime.ServerId, StringComparison.OrdinalIgnoreCase) || !view.EditEligible) ? "ログイン中は拠点またはスキルツリーワールドでのみ編集できます。" : null;
        var now = DateTime.UtcNow;
        ...
        if (reason is null && await dbContext.SkillTreeOperations.AnyAsync(x => x.AccountId == accountId && (x.Status == SkillTreeOperationStatuses.PendingOnline || x.Status == SkillTreeOperationStatuses.PendingOffline || x.Status == SkillTreeOperationStatuses.Claimed))) return null;
    ...
    public async Task<bool> CompleteFromSnapshotAsync(Guid accountId, PlayerStateSkillTreeOperationSection section, DateTime now)
    {
        ...
        if (section.EvaluatedView.HasValue && IsViewPayload(section.EvaluatedView.Value)) await UpsertViewAsync(runtime, accountId, state.Version, section.FinalEvaluationFingerprint, true, false, section.EvaluatedView.Value, now);
    ...
    private async Task<SkillTreeEditorResponse> BuildEditorAsync(AccountEntity account, AccountSkillTreeStateEntity? state, string? targetServerId, Guid? actorUserId)
    {
        ...
        var pending = await dbContext.SkillTreeOperations.AsNoTracking().Where(x => x.AccountId == account.Uuid && (x.Status == SkillTreeOperationStatuses.PendingOnline || x.Status == SkillTreeOperationStatuses.PendingOffline || x.Status == SkillTreeOperationStatuses.Claimed)).OrderByDescending(x => x.CreatedAtUtc).ToArrayAsync();
    ...
    private async Task UpsertViewAsync(SkillTreeServerRuntimeEntity runtime, Guid accountId, int version, string fingerprint, bool eligible, bool offlineConfirmed, JsonElement view, DateTime now) { var item = await dbContext.SkillTreeServerPlayerViews.FindAsync(runtime.ServerId, accountId); if (item is null) { item = new SkillTreeServerPlayerViewEntity { ServerId = runtime.ServerId, AccountId = accountId }; await dbContext.SkillTreeServerPlayerViews.AddAsync(item); } item.ServerSessionId = runtime.ServerSessionId; item.DefinitionGenerationId = runtime.DefinitionGenerationId; item.PlayerStateVersion = version; item.EvaluationFingerprint = fingerprint; item.EditEligible = eligible; item.OfflineConfirmed = offlineConfirmed; item.ViewJson = view.GetRawText(); item.LastSeenUtc = now; }
    ...
    private async Task ExpireAsync() => await dbContext.SkillTreeOperations.Where(x => (x.Status == SkillTreeOperationStatuses.PendingOnline || x.Status == SkillTreeOperationStatuses.PendingOffline || x.Status == SkillTreeOperationStatuses.Claimed) && x.ExpiresAtUtc <= DateTime.UtcNow).ExecuteUpdateAsync(x => x.SetProperty(v => v.Status, SkillTreeOperationStatuses.Expired).SetProperty(v => v.CompletedAtUtc, DateTime.UtcNow));
