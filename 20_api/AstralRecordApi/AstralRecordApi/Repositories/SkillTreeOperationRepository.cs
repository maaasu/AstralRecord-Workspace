...
public sealed partial class SkillTreeOperationRepository(AstralRecordDbContext dbContext, INetworkRuntimeService networkRuntimeService) : ISkillTreeOperationRepository
{
    ...
    private static bool ValidOperation(SkillTreeOperationCreateRequest x) => x.OperationId != Guid.Empty && x.ActorUserId != Guid.Empty
        ...
        && (x.Action is "UNLOCK" or "RELOCK" && x.Changes is null && !string.IsNullOrWhiteSpace(x.NodeId) && x.NodeId.Trim().Length <= 200 || x.Action == "BATCH" && x.NodeId == "batch" && x.Changes is { Count: >= 1 and <= 512 } && x.Changes.All(c => c is not null && c.Action is "UNLOCK" or "RELOCK" && !string.IsNullOrWhiteSpace(c.NodeId) && c.NodeId.Trim().Length <= 200 && c.SourceClassId?.Length is not > 100) && x.Changes.Select(c => c.NodeId.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).Count() == x.Changes.Count) && x.SourceClassId?.Length is not > 100;
    ...
    private static SkillTreeServerRuntimeResponse Runtime(SkillTreeServerRuntimeEntity x) => new() { ServerId = x.ServerId, ServerSessionId = x.ServerSessionId, PublicationRevision = x.PublicationRevision, DefinitionGenerationId = x.DefinitionGenerationId, Ready = x.Ready, LastSeenUtc = x.LastSeenUtc };
