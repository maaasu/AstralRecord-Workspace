...
class EquipmentProcessingMenuScreenViewTest extends MockBukkitTestBase {
    ...
    void rendersModeIdentityAndMaterialListEntryWithActualMaterialIcons() {
        ...
        view.render(
            ...
            EquipmentProcessingDisplayState.ENHANCEMENT,
            null,
        ...
        assertEquals("装備加工｜強化", view.processingTitle(EquipmentProcessingDisplayState.ENHANCEMENT));
        ...
        view.render(
            ...
            EquipmentProcessingDisplayState.REPAIR,
            null,
            new ItemStack(Material.BOOK),
        ...
        assertEquals("装備加工｜修理", view.processingTitle(EquipmentProcessingDisplayState.REPAIR));
     ...
     * 設計入力: 00_docs/10_Plugin設計書/feature/09-menu/3-メソッド仕様/09_3-GUI・View.md
     * 章・見出し: # 09_3-GUI・View > ## 5.5. 装備加工画面
     * 検証契約: 状態変化が次の実行内容になった強化画面は、強化タブを保ったままタイトル・水色の帯・結晶アイコンで状態変化中を常時表示する。
     */
    @Test
    void rendersTranscendenceAsAnEnhancementState() {
        Inventory inventory = Bukkit.createInventory(null, BaseMenuScreenView.SIZE);
        EquipmentProcessingMenuScreenView view = new EquipmentProcessingMenuScreenView();
        ...
        view.render(
            inventory,
            EquipmentProcessingMode.ENHANCEMENT,
            EquipmentProcessingDisplayState.TRANSCENDENCE,
            null,
            new ItemStack(Material.END_CRYSTAL),
            new ItemStack(Material.NETHER_STAR),
            List.of(new ItemStack(Material.DIAMOND)),
            new ItemStack(Material.CHEST),
            new ItemStack(Material.LIME_CONCRETE)
        );

        assertEquals("装備加工｜状態変化", view.processingTitle(EquipmentProcessingDisplayState.TRANSCENDENCE));
        assertEquals(Material.CYAN_STAINED_GLASS_PANE, inventory.getItem(1).getType());
        assertEquals(Material.END_CRYSTAL, inventory.getItem(EquipmentProcessingMenuScreenView.MODE_EMBLEM_SLOT).getType());
        assertEquals(Material.ENCHANTING_TABLE, inventory.getItem(EquipmentProcessingMenuScreenView.ENHANCEMENT_TAB_SLOT).getType());

        view.renderMaterialList(inventory, EquipmentProcessingDisplayState.TRANSCENDENCE, List.of(new ItemStack(Material.DIAMOND)), 0, 1);

        assertEquals("装備加工｜状態変化｜必要素材一覧", view.materialListTitle(EquipmentProcessingDisplayState.TRANSCENDENCE));
        assertEquals(Material.END_CRYSTAL, inventory.getItem(EquipmentProcessingMenuScreenView.MATERIAL_LIST_BACK_SLOT).getType());
    }
    ...
    void rendersAllMaterialsInPagedMaterialList() {
        ...
        view.renderMaterialList(inventory, EquipmentProcessingDisplayState.ENHANCEMENT, materials, 0, 2);

        assertEquals("装備加工｜強化｜必要素材一覧", view.materialListTitle(EquipmentProcessingDisplayState.ENHANCEMENT));
        ...
        view.renderMaterialList(inventory, EquipmentProcessingDisplayState.ENHANCEMENT, materials, 1, 2);
