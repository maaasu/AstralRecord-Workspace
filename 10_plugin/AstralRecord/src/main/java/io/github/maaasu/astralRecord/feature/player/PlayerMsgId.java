package io.github.maaasu.astralRecord.feature.player;

/**
 * プレイヤー向けメッセージIDを定義する enum。
 * メッセージID形式: {Type}_{Number}
 * <p>
 * {Type}
 * - P: Player（プレイヤー向け汎用メッセージ）
 * <p>
 * メッセージ文言は {@code player.properties} で管理します。
 * <p>
 * 使用例:
 * <pre>
 * // パラメータあり
 * PlayerMessageService.getInstance().send(astPlayer, PlayerMsgId.P_5945);
 * </pre>
 */
public enum PlayerMsgId {

    // ==================== feature ====================

    // region /feature/player/model/AstPlayer.kt 5000-5049
    /**
     * AstralRecord へようこそ、%s さん！
     */
    // endregion

    // region /test/TestCommand.java 5050-5059
    /**
     * 固定スキンを表示できない
     */
    P_5050(5050),
    /**
     * 指定したプレイヤースキンを10秒間表示する
     */
    P_5051(5051),
    /**
     * world not found: {0}
     */
    P_5052(5052),
    /**
     * world={0}
     */
    P_5053(5053),
    /**
     * entities={0}, displays={1}, itemDisplays={2}, textDisplays={3}
     */
    P_5054(5054),
    /**
     * skilltreeDisplays={0}, admin={1}, node={2}, edge={3}
     */
    P_5055(5055),
    // endregion

    // region /temp/command/TempCommand.java 5080-5089
    /**
     * [TEMP] テンポラリコマンドを実行しました。実行者: {0}
     */
    /**
     * [TEMP] テキスト表示を削除しました。
     */
    /**
     * [TEMP] BlockDisplay に使えるブロック icon がありません: item={0}, icon={1}
     */
    // endregion

    // region /feature/player/event/PlayerJoinEventHandler.java 5070-5079
    /**
     * あなたはpermissionレベル{0}の管理者です。OP権限が付与されました。
     */
    P_5070(5070),
    /**
     * プレイヤーデータをロード中です。完了までその場でお待ちください。
     */
    P_5071(5071),
    /**
     * プレイヤーデータのロードが完了しました: ({0}ms)
     */
    P_5072(5072),
    /**
     * プレイヤーデータロード中の title 表示です。
     */
    P_5073(5073),
    /**
     * ログインボーナス処理に失敗しました。時間をおいて再試行してください。
     */
    P_5074(5074),
    /**
     * 本日のログインボーナスは受け取り済みです。明日また受け取れます。
     */
    P_5075(5075),
    /**
     * プレイヤーが冒険の世界へ参加したことを全体へ通知する。プレイヤー名は情報 GUI を開くクリック導線。
     */
    P_5076(5076),
    /**
     * プレイヤーが冒険の世界から退出したことを全体へ通知する。プレイヤー名は情報 GUI を開くクリック導線。
     */
    P_5077(5077),
    /**
     * 金曜日のログインボーナスを既存のストレージ entry へ格納した通知。{0}: アイテム表示名、{1}: 格納数量
     */
    P_5078(5078),
    /**
     * 初参加者へガイドを開くよう促す title。
     */
    P_5079(5079),
    // endregion

    // region /feature/player/event/PlayerJoinEventHandler.java 5083-5084
    /**
     * 本サーバーの利用規約確認を促す参加時案内。
     */
    P_5083(5083),
    /**
     * 同一IPに登録済みの他ユーザーがいる場合の監視案内。
     */
    P_5084(5084),
    // endregion

    // region /feature/player/death 5080-5089
    /**
     * 死亡しました
     */
    P_5080(5080),
    /**
     * リスポーンまで {0} 秒
     */
    P_5081(5081),
    /**
     * {0} は死亡しました
     */
    P_5082(5082),
    // endregion

    // region /infrastructure/command/AstCommand.java 5060-5069
    /**
     * このコマンドはプレイヤーのみ使用できます。
     */
    P_5060(5060),
    /**
     * このコマンドを実行する権限がありません。
     */
    P_5061(5061),
    /**
     * コマンドの実行中にエラーが発生しました。詳細はサーバーログを確認してください。
     */
    P_5062(5062),
    /**
     * 引数が不足しています。
     */
    P_5063(5063),
    /**
     * 使用方法: {0}
     */
    P_5064(5064),
    /**
     * 現在の account mode ではこのコマンドを使用できません。
     */
    P_5065(5065),
    // endregion

    // region /core/command/MasterDataCommand.java 5095-5099
    /**
     * マスターデータの再読込を開始しました。完了までお待ちください。
     */
    P_5095(5095),
    /**
     * マスターデータはすでに再読込中です。
     */
    P_5096(5096),
    /**
     * マスターデータの再読込が完了しました（{0} 件）。
     */
    P_5097(5097),
    /**
     * マスターデータの再読込に失敗しました。詳細はサーバーログを確認してください。
     */
    P_5098(5098),
    // endregion

    // region /feature/status/command/StatusCommand.java 5100-5149
    /**
     * ステータス: {0}
     */
    P_5100(5100),
    /**
     * {0}: {1}
     */
    P_5101(5101),
    /**
     * ステータスを再計算しました。
     */
    P_5102(5102),
    /**
     * {0}: 基礎 {1} / 補正 {2} / 合計 {3}
     */
    P_5103(5103),
    /**
     * 最終更新: {0}
     */
    P_5104(5104),
    /**
     * 現在HP/MP: {0}/{1} | {2}/{3}
     */
    P_5105(5105),
    /**
     * 現在ENG: {0}/{1}
     */
    P_5106(5106),
    /**
     * セット効果:
     */
    P_5107(5107),
    /**
     * {0}: 装備数 {1} / 発動 {2}
     */
    P_5108(5108),
    /**
     * ステータスバフを付与しました: {0} -> {1} {2} / {3}秒
     */
    P_5109(5109),
    /**
     * ステータスIDが不正です: {0}
     */
    P_5110(5110),
    /**
     * 上昇値は正の数値、持続時間は1〜{0}秒で指定してください。
     */
    P_5111(5111),
    /**
     * 対象プレイヤーがオンラインではありません: {0}
     */
    P_5112(5112),
    /**
     * 管理者からステータスバフを得ました: {0} {1} / {2}秒
     */
    P_5113(5113),
    /**
     * NPCの回復アクションで現在のリソースを全回復した。
     */
    P_5114(5114),
    // endregion

    // region /feature/item/command/ItemCommand.java 5200-5249
    /**
     * 無効なカテゴリです: {0}
     */
    /**
     * アイテムが見つかりません: category={0}, id={1}
     */
    P_5201(5201),
    /**
     * アイテム: {0} ({1})
     */
    /**
     * カテゴリ: {0} / レアリティ: {1} / 売却値: {2}
     */
    /**
     * 説明はありません。
     */
    /**
     * 説明:
     */
    /**
     * - {0}
     */
    /**
     * {0} は消耗品ではないため使用できません。
     */
    /**
     * {0} を使用しました。適用: {1}件 / スキップ: {2}件
     */
    /**
     * ロードしました: {0}/{1} ({2})
     */
    P_5209(5209),
    /**
     * ロード済みアイテムはありません。
     */
    /**
     * ロード済みアイテム一覧 ({0}件)
     */
    /**
     * [{0}] {1} - {2}
     */
    /**
     * 指定IDのロード済みアイテムが見つかりません: {0}
     */
    P_5213(5213),
    P_5257(5257),
    P_5258(5258),
    P_5259(5259),
    P_5263(5263),
    P_5264(5264),
    P_5265(5265),
    P_5266(5266),
    P_5267(5267),
    P_5268(5268),
    P_5269(5269),
    P_5270(5270),
    /** 消耗品の重要保存に失敗した。 */
    P_5273(5273),
    /** フックショット装填に必要なフック不足。 */
    P_5271(5271),
    /** フックショットの照準が命中しなかった。 */
    P_5272(5272),
    /**
     * 同一IDが複数カテゴリに存在します。カテゴリを指定してください: {0}
     */
    /**
     * 候補: [{0}] {1} - {2}
     */
    /**
     * icon={0}, schema={1}, customModelData={2}
     */
    /**
     * 取引不可={0}, 売却不可={1}
     */
    /**
     * maxStack={0}
     */
    /**
     * [bundle]
     */
    /**
     * lootTableId={0}
     */
    /**
     * onUse: sound={0}, effect={1}, particle={2}
     */
    /**
     * [currency]
     */
    /**
     * type={0}, group={1}, expiresAt={2}
     */
    /**
     * [equipment]
     */
    /**
     * slot={0}, handType={1}, requiredLevel={2}
     */
    /**
     * requiredClasses={0}
     */
    /**
     * stat: {0} {1} {2}
     */
    /**
     * durability: max={0}, consume={1}
     */
    /**
     * onUse: leftCooldown={0}, leftSkill={1}, rightCooldown={2}, rightSkill={3}
     */
    /**
     * skills={0}
     */
    /**
     * [{0}] 追加セクションはありません。
     */
    /**
     * [{0}] 未対応カテゴリです。
     */
    /**
     * [{0}] セクション定義がありません。
     */
    /**
     * [consumable]
     */
    /**
     * onUse: sound={0}, effect={1}, amount={2}
     */
    /**
     * effects={0}
     */
    /**
     * effect: type={0}, rate={1}, status={2}, value={3}, isPercent={4}, buffId={5}
     */
    /**
     * [lore]
     */
    /**
     * アイテムを付与しました: {0} x{1}
     */
    P_5240(5240),
    /**
     * インベントリに空きがありません。
     */
    P_5241(5241),
    P_5242(5242),
    P_5243(5243),
    /**
     * インベントリの空き slot が残り {0} です。
     */
    P_5244(5244),
    P_5245(5245),
    P_5246(5246),
    P_5247(5247),
    P_5248(5248),
    P_5249(5249),
    /**
     * 装備の耐久値が 0 になり、使用できなくなりました: {0}
     */
    P_5279(5279),
    /**
     * 武器の耐久値が指定割合以下になりました: {0}, {1}
     */
    P_5282(5282),
    /** 装備中の防具・アクセサリに破損があります: {0} */
    P_5283(5283),
    /** 必要プレイヤーレベル未達。 */
    P_5284(5284),
    /** 必要クラス不一致。 */
    P_5285(5285),
    /** 必要クラスレベル未達。 */
    P_5286(5286),
    /** 装備状態変化成功。 */
    P_5287(5287),
    /** このオーブで操作できる装備がない。 */
    P_5288(5288),
    /** 使用起点のオーブを所持していない。 */
    P_5289(5289),
    /** 対象装備の再検証失敗。 */
    P_5290(5290),
    /** 状態変化素材またはゴールド不足。 */
    P_5291(5291),
    /** オーブによる耐久回復成功。 */
    P_5292(5292),
    /** オーブによるエンチャント成功。 */
    P_5293(5293),
    /** 重複禁止等により付与可能なエンチャント候補がない。 */
    P_5294(5294),
    /** 装備操作失敗と支払い返却。 */
    P_5295(5295),
    /** 装備操作の応答待ちを保留へ切り替えた。 */
    P_5296(5296),
    /** オーブ複数回使用の集計結果。 */
    P_5297(5297),
    // endregion

    // region /feature/user/command 5300-5319
    /**
     * 対象プレイヤーまたはUUIDが見つかりません: {0}
     */
    P_5300(5300),
    /**
     * permission は名称（PLAYER/ADMIN）または 0 / 99 のいずれかで指定してください。
     */
    P_5301(5301),
    /**
     * UserService が初期化されていません。
     */
    P_5302(5302),
    /**
     * ユーザー更新後の取得に失敗しました: {0}
     */
    P_5303(5303),
    /**
     * permission を更新しました: {0} = {1}
     */
    P_5304(5304),
    /**
     * このコマンドをコンソールから実行する場合は対象プレイヤーの指定が必要です。
     */
    P_5305(5305),
    /** 同一対象の権限更新が処理中であることを通知します。 */
    P_5306(5306),
    /** BAN 対象者の接続を拒否するメッセージ。 */
    P_5307(5307),
    /** 無期限 BAN の設定完了通知。{0}: Minecraft ID */
    P_5308(5308),
    /** BAN 更新処理中の通知。 */
    P_5309(5309),
    /** BAN コマンドの引数不正通知。 */
    P_5310(5310),
    /** 有期限 BAN の日数不正通知。 */
    P_5311(5311),
    /** 有期限 BAN の設定完了通知。{0}: Minecraft ID、{1}: 日数、{2}: 期限 */
    P_5312(5312),
    /** プレイヤーデータ初期化失敗による接続拒否通知。 */
    P_5313(5313),
    // endregion

    // region /feature/guide 5180-5189
    /** ガイド手順達成通知。{0}: 手順本文 */
    P_5180(5180),
    /** ガイド全体達成通知。{0}: ガイド名 */
    P_5181(5181),
    /** ガイドの案内対象 NPC の座標通知。{0}: NPC名、{1}: X、{2}: Y、{3}: Z */
    P_5182(5182),
    /** ガイドの案内対象 NPC が見つからない場合の通知。 */
    P_5183(5183),
    /** ガイドの案内アクションを実行できない場合の通知。 */
    P_5184(5184),
    // endregion

    // region /feature/playersetting 5320-5329
    P_5320(5320),
    P_5321(5321),
    P_5322(5322),
    P_5323(5323),
    P_5324(5324),
    P_5325(5325),
    P_5326(5326),
    // endregion

    // region /feature/account/command 5330-5349
    P_5330(5330),
    P_5331(5331),
    P_5332(5332),
    P_5333(5333),
    P_5334(5334),
    /** 削除されたアカウントスロット。{0}: スロット番号 */
    P_5335(5335),
    /** 管理者向けアカウント削除成功。{0}: スロット番号 {1}: アカウント名 */
    P_5336(5336),
    /** アカウントスロット指定が不正。 */
    P_5337(5337),
    /** アカウント削除処理中。 */
    P_5338(5338),
    /** アカウント削除後の再ロード失敗による切断。 */
    P_5339(5339),
    /** アカウント切替サービスが初期化されていない。 */
    P_5340(5340),
    /** アカウント切替処理中。 */
    P_5341(5341),
    /** アカウント切替成功。{0}: スロット番号 {1}: アカウント名 */
    P_5342(5342),
    /** 未作成スロットを作成して切替成功。{0}: スロット番号 {1}: アカウント名 */
    P_5343(5343),
    /** カーソル上のアイテムがあるためアカウントを切り替えられない。 */
    P_5344(5344),
    /** 管理者が対象プレイヤーの既存アカウントを切り替えた。{0}: プレイヤー名 {1}: スロット番号 {2}: アカウント名 */
    P_5345(5345),
    /** 管理者が対象プレイヤーの未作成スロットを作成して切り替えた。{0}: プレイヤー名 {1}: スロット番号 {2}: アカウント名 */
    P_5346(5346),
    /** アカウント名が英数字またはアンダースコアの 3-50 文字ではない。 */
    P_5347(5347),
    /** アカウント名が既に使用されている。 */
    P_5348(5348),
    /** アカウント名変更成功。{0}: アカウント表示名 */
    P_5349(5349),
    // endregion

    // region /feature/combat 5350-5359
    /** 与ダメージ詳細。{0}:反映先と値 {1}:攻撃種別 {2}:属性 {3}:攻撃力 {4}:元防御 {5}:有効防御 {6}:耐性表示 {7}:命中率 {8}:命中 {9}:回避 {10}:会心表示 {11}:HP表示 */
    P_5350(5350),
    /** 与ダメージ回避。{0}:命中率 {1}:命中 {2}:回避 */
    P_5351(5351),
    /** 被ダメージ詳細。{0}:反映先と値 {1}:攻撃種別 {2}:属性 {3}:攻撃力 {4}:元防御 {5}:有効防御 {6}:耐性表示 {7}:命中率 {8}:命中 {9}:回避 {10}:会心表示 {11}:HP表示 */
    P_5352(5352),
    /** 被ダメージ回避。{0}:命中率 {1}:命中 {2}:回避 */
    P_5353(5353),
    /** 与えたHP回復の詳細。{0}:実回復量 {1}:回復対象プレイヤー {2}:回復手段 */
    P_5354(5354),
    /** 被ったHP回復の詳細。{0}:実回復量 {1}:回復元プレイヤー {2}:回復手段 */
    P_5355(5355),
    /** 自己HP回復の詳細。{0}:実回復量 {1}:回復手段 */
    P_5356(5356),
    /** 通常攻撃劣化の開始案内。 */
    P_5357(5357),
    // endregion

    // region /feature/inventory/command/InventoryCommand.java 5250-5269
    /**
     * 現在の mode では AstralRecord インベントリを反映しません。
     */
    P_5250(5250),
    /**
     * 無効なインベントリタイプです。指定可能: {0}
     */
    /**
     * InventoryService が初期化されていません。
     */
    P_5252(5252),
    /** ストレージからのアイテム取得通知。{0}: アイテム表示名、{1}: 取得数量 */
    P_5253(5253),
    // endregion

    // region /feature/inventory/service/InventoryAutoSaveTask.java 5280-5289
    /** オートセーブ開始通知。 */
    P_5280(5280),
    /** オートセーブ正常終了通知。 */
    P_5281(5281),
    /**
     * インベントリを切り替えました: {0}
     */
    // endregion

    // region /feature/resourcepack/service/ResourcePackService.java 5550-5599
    /**
     * リソースパックのダウンロードに失敗しました。
     */
    P_5550(5550),
    /**
     * リソースパックのURLが不正です。
     */
    P_5551(5551),
    /**
     * クライアント側でリソースパックの再読み込みに失敗しました。
     */
    P_5552(5552),
    /**
     * リソースパックが拒否されました。
     */
    P_5553(5553),
    // endregion

    // region /feature/menu 5600-5609
    /** 定期ガイド誘導メッセージ。 */
    P_5600(5600),
    /**
     * ショートカット設定の保存に失敗しました。設定を初期化しました。
     */
    /**
     * ゴミ箱から {0} 件のアイテムを削除しました。
     */
    P_5601(5601),
    /**
     * ゴミ箱GUI内の {0} 件のアイテムをインベントリに戻しました。
     */
    P_5602(5602),
    P_5603(5603),
    /**
     * {0} 件のアイテムを売却し、{1} ゴールドを受け取りました。
     */
    P_5604(5604),
    /**
     * このアイテムは売却できません。
     */
    P_5605(5605),
    /**
     * 売却GUI内の {0} 件のアイテムをインベントリに戻しました。
     */
    P_5606(5606),
    /**
     * 拠点帰還の待機を開始しました。
     */
    P_5607(5607),
    /**
     * 拠点帰還をキャンセルしました。
     */
    P_5608(5608),
    /**
     * 拠点帰還に必要なゴールドが不足しています。
     */
    P_5609(5609),
    /**
     * 拠点へ帰還しました。
     */
    P_5610(5610),
    /**
     * 拠点へ帰還できませんでした。
     */
    P_5611(5611),
    /**
     * 拠点帰還中のタイトル表示です。
     */
    P_5612(5612),
    /**
     * 拠点帰還中のサブタイトル表示です。
     */
    P_5613(5613),
    /**
     * 拠点帰還中のボスバー表示です。
     */
    P_5614(5614),
    /**
     * 拠点帰還キャンセル時のタイトル表示です。
     */
    P_5615(5615),
    /**
     * 拠点帰還キャンセル時のサブタイトル表示です。
     */
    P_5616(5616),
    /**
     * 拠点帰還完了時のタイトル表示です。
     */
    P_5617(5617),
    /**
     * 拠点帰還完了時のサブタイトル表示です。
     */
    P_5618(5618),
    /**
     * オーバーワールドのスポーン地点から拠点へ無償帰還した時のメッセージです。
     */
    P_5619(5619),
    // endregion

    // region /feature/mail 5620-5629
    /**
     * メール「{0}」を既読にし、報酬を受け取りました。
     */
    P_5620(5620),
    /**
     * メールを一覧から削除しました。
     */
    P_5621(5621),
    /**
     * メールが見つかりません。
     */
    P_5622(5622),
    /**
     * メール「{0}」の報酬を受け取れませんでした。
     */
    P_5623(5623),
    /** 参加時の未読メール件数通知。 */
    P_5624(5624),
    /** メール報酬の重要保存に失敗した。 */
    P_5625(5625),
    // endregion

    // region /feature/mob 5700-5728
    /**
     * Mob テンプレートをロードしました: {0}件
     */
    P_5700(5700),
    /**
     * ロード済み Mob はありません。
     */
    P_5701(5701),
    /**
     * ロード済み Mob: {0}
     */
    P_5702(5702),
    /**
     * Mob テンプレートが見つかりません: {0}
     */
    P_5703(5703),
    /**
     * Mob を出現させました: {0} / instance={1}
     */
    P_5704(5704),
    /**
     * 削除対象の Mob が見つかりません: {0}
     */
    P_5705(5705),
    /**
     * Mob を削除しました: {0}件
     */
    P_5706(5706),
    /**
     * 管理者権限を持つプレイヤーのみ使用できます。
     */
    P_5707(5707),
    /**
     * スポナー定義が見つかりません: {0}
     */
    P_5708(5708),
    /**
     * スポナーを登録しました: {0}
     */
    P_5709(5709),
    /**
     * スポナー登録を削除しました。
     */
    P_5710(5710),
    /**
     * スポナー座標を登録できません: {0}
     */
    P_5711(5711),
    /**
     * スポナー定義をロードしました: {0}件
     */
    P_5712(5712),
    /**
     * スポナー定義: {0} / 登録座標: {1}件
     */
    P_5713(5713),
    /**
     * スポナーアイテムを付与しました: {0} x{1}
     */
    P_5714(5714),
    /**
     * NPC 配置をロードしました: {0}件
     */
    P_5715(5715),
    /**
     * NPC 配置: {0}件
     */
    P_5716(5716),
    /**
     * 座標は数値で指定してください。
     */
    P_5717(5717),
    /**
     * NPC を配置しました: {0} world={1} x={2} y={3} z={4}
     */
    P_5718(5718),
    /**
     * 管理者モードのプレイヤーのみ使用できます。
     */
    P_5719(5719),
    /**
     * 採集定義とスポナー定義をロードしました: 採集 {0}件 / スポナー {1}件
     */
    P_5720(5720),
    /**
     * 採集定義: {0} / 出現中: {1}件
     */
    P_5721(5721),
    /**
     * 採集定義が見つかりません: {0}
     */
    P_5722(5722),
    /**
     * 採集オブジェクトを出現させました: {0} / instance={1}
     */
    P_5723(5723),
    /**
     * 採集スポナー定義をロードしました: {0}件
     */
    P_5724(5724),
    /**
     * 採集スポナー定義: {0} / 登録座標: {1}件
     */
    P_5725(5725),
    /**
     * NPC配置を削除しました: {0} / {1}件
     */
    P_5726(5726),
    /**
     * 削除対象のNPC配置が見つかりません: {0}
     */
    P_5727(5727),
    /**
     * レアドロップ獲得本人向け通知。
     */
    P_5728(5728),
    /** 採集スポナーの管理表示ラベル。 */
    P_5729(5729),
    /** モブスポナーの管理表示ラベル。 */
    P_5730(5730),
    /** 装備ドロップの重要保存に失敗した。 */
    P_5731(5731),
    /** 通常ドロップ獲得本人向け通知。 */
    P_5732(5732),
    /** 他プレイヤー向けレアドロップ全体通知。 */
    P_5733(5733),
    /** ツールのギャザリングレベルが採集対象の必要レベルに満たない。 */
    P_5734(5734),
    // endregion

    // region /feature/textdisplay/command/TextDisplayCommand.java 5770-5779
    /**
     * 固定 TextDisplay を配置しました: {0} world={1} x={2} y={3} z={4}
     */
    P_5770(5770),
    /**
     * 固定 TextDisplay を削除しました: {0}
     */
    P_5771(5771),
    /**
     * 固定 TextDisplay: {0}件 {1}
     */
    P_5772(5772),
    /**
     * 固定 TextDisplay 配置をロードしました: {0}件
     */
    P_5773(5773),
    /**
     * ID は半角英数字、アンダースコア、ハイフンのみ使用できます。
     */
    P_5774(5774),
    /**
     * 固定 TextDisplay が見つかりません: {0}
     */
    P_5775(5775),
    /**
     * 固定 TextDisplay は登録されていません。
     */
    P_5776(5776),
    // endregion

    // region /feature/world/service/OverworldTeleportService.java 5777-5779
    /**
     * 転送先ワールドの必要アイテムを所持していない。
     */
    P_5777(5777),
    // endregion

    // region /feature/player/service/PlayerRegionService.java 5780-5789
    /**
     * 地域切り替えタイトル: {0}
     */
    P_5780(5780),
    /**
     * 地域移動
     */
    P_5781(5781),
    // endregion

    // region /feature/world/command/WorldCommand.java 5750-5769
    P_5751(5751),
    P_5752(5752),
    P_5753(5753),
    P_5754(5754),
    P_5755(5755),
    P_5756(5756),
    P_5757(5757),
    P_5758(5758),
    P_5759(5759),
    P_5760(5760),
    P_5761(5761),
    P_5762(5762),
    P_5763(5763),
    P_5764(5764),
    P_5765(5765),
    P_5766(5766),
    P_5767(5767),
    P_5768(5768),
    P_5769(5769),
    // endregion

    // region /feature/hotbaraction/event/HotbarActionEventHandler.java 6000-6009
    /**
     * [DEBUG] ホットバー左クリック: {0}
     */
    /**
     * [DEBUG] ホットバー右クリック: {0}
     */
    // endregion

    // region /feature/skill 5800-5819
    /**
     * 必要レベルが不足しています。
     */
    P_5800(5800),
    /**
     * MP が不足しています。
     */
    P_5801(5801),
    /**
     * クールダウン中です。
     */
    P_5802(5802),
    /**
     * 未定義のスキルです: {0}
     */
    P_5803(5803),
    /**
     * 実行クラスが登録されていません: {0}
     */
    P_5804(5804),
    /**
     * スキル発動に失敗しました。
     */
    P_5805(5805),
    P_5806(5806),
    /**
     * [DEBUG] アクションリング発動: bindCount={0}, selectedSlot={1}
     */
    P_5807(5807),
    /**
     * スキルプリセット {0} に切り替えました。
     */
    P_5808(5808),
    /**
     * 未習得スキルです: {0}
     */
    P_5809(5809),
    /**
     * すでに別のスキルを詠唱中です。
     */
    P_5810(5810),
    /**
     * 詠唱中: {0} {1}s
     */
    P_5811(5811),
    /**
     * クラスを変更しました: {0} -> {1}
     */
    P_5812(5812),
    /**
     * クラスが見つかりません: {0}
     */
    P_5813(5813),
    /**
     * 対象プレイヤーが見つかりません: {0}
     */
    P_5814(5814),
    P_5815(5815),
    P_5816(5816),
    P_5817(5817),
    P_5818(5818),
    P_5819(5819),
    P_5820(5820),
    P_5821(5821),
    P_5822(5822),
    P_5823(5823),
    P_5824(5824),
    P_5825(5825),
    P_5826(5826),
    P_5827(5827),
    P_5828(5828),
    P_5829(5829),
    P_5830(5830),
    P_5831(5831),
    P_5832(5832),
    P_5833(5833),
    P_5834(5834),
    P_5835(5835),
    P_5836(5836),
    P_5837(5837),
    P_5838(5838),
    P_5839(5839),
    P_5840(5840),
    P_5841(5841),
    P_5842(5842),
    P_5843(5843),
    P_5844(5844),
    P_5845(5845),
    P_5846(5846),
    P_5847(5847),
    P_5848(5848),
    P_5849(5849),
    /**
     * クラスレベルを管理コマンドで設定した結果を表示します。
     */
    P_5850(5850),
    /**
     * 管理コマンド専用クラスであることを案内します。
     */
    P_5851(5851),
    /**
     * プレイヤーレベルを管理コマンドで設定した結果を表示します。
     */
    P_5852(5852),
    /**
     * アカウント進行サービスが利用できないことを通知します。
     */
    P_5853(5853),
    /**
     * アクションリングを開いた直後にスキル選択操作を案内します。
     */
    P_5854(5854),
    /**
     * アクションリングで選択したスキルの発動操作を案内します。
     */
    P_5855(5855),
    /**
     * アクションスキル未設定時にスキルマネージャーへのクリック導線を案内します。
     */
    P_5856(5856),
    /**
     * スキルをバインドするには先に設定先スロットを選択する必要があることを案内します。
     */
    P_5858(5858),
    /**
     * 選択したシジルが対象スキルへ許可されていないことを通知します。
     */
    P_5859(5859),
    /**
     * シジルを装着できる空き枠がないことを通知します。
     */
    P_5860(5860),
    /**
     * 同じシジル装着グループを重複して装着できないことを通知します。
     */
    P_5861(5861),
    /**
     * 選択した合成素材が現在のスキル状態に使用できないことを通知します。
     */
    P_5862(5862),
    /**
     * 所持済みでも現在のクラスまたはツリーで使用許可がないスキルを通知します。
     */
    P_5863(5863),
    /**
     * スキル合成の予期しない失敗時に素材が未消費であることを通知します。
     */
    P_5864(5864),
    /**
     * 種別に合う自動設定先の空きスロットがないことを通知します。
     */
    P_5865(5865),
    /** スキル忘却の成功通知。{0}: スキル名 */
    P_5866(5866),
    /** スキル忘却の失敗通知。 */
    P_5867(5867),
    /** 忘却の代償に必要なアストラルドが不足していることを通知します。 */
    P_5868(5868),
    /** 代償付き忘却の成功通知。{0}: スキル名 */
    P_5869(5869),
    /** 忘却時に返却する購入素材を準備できないことを通知します。 */
    P_5870(5870),
    /** アクションリング長押し選択で右クリック解除を案内します。 */
    P_5871(5871),
    /** シジルオーブで操作可能な習得済みスキルがない。 */
    P_5872(5872),
    /** シジル装着または脱着の正本更新に失敗した。 */
    P_5873(5873),
    /** スキル操作の応答待ちを保留へ切り替えた。 */
    P_5874(5874),
    /** スキルノード強調表示の変更結果を通知します。 */
    P_5875(5875),
    /** スキルツリーのステータス絞り込み変更結果を通知します。 */
    P_5876(5876),
    /** ステータス絞り込みに指定されたIDが不正であることを通知します。 */
    P_5877(5877),
    /** スキルツリーコマンドの使用方法を表示します。 */
    P_5878(5878),
    /** バスティオンストライクが発動して攻撃を無効化したことを通知します。 */
    P_5880(5880),
    /** パラディンシールドの発動に必要な現在シールドが不足している。 */
    P_5881(5881),
    /** ホーリーコントロールの発動に必要な現在HPが不足している。 */
    P_5882(5882),
    /** ホーリーコントロールの操作対象となる聖柱が範囲内にない。 */
    P_5883(5883),
    // endregion

    // region /feature/party 5900-5929
    P_5900(5900),
    P_5901(5901),
    P_5902(5902),
    P_5903(5903),
    P_5904(5904),
    P_5905(5905),
    P_5906(5906),
    P_5907(5907),
    P_5908(5908),
    P_5909(5909),
    P_5910(5910),
    P_5911(5911),
    P_5912(5912),
    P_5913(5913),
    P_5914(5914),
    P_5915(5915),
    P_5916(5916),
    P_5917(5917),
    P_5918(5918),
    P_5919(5919),
    P_5920(5920),
    P_5921(5921),
    P_5922(5922),
    P_5923(5923),
    P_5924(5924),
    P_5925(5925),
    /** パーティーチャットモードを有効にした。 */
    P_5926(5926),
    /** パーティーチャットモードを無効にした。 */
    P_5927(5927),
    /** 管理者向けパーティー一覧の見出し。 */
    P_5928(5928),
    /** 管理者向けパーティー一覧の各行。 */
    P_5929(5929),
    // endregion

    // region /feature/shop 5930-5939
    P_5930(5930),
    // endregion

    // region /feature/trade 6200-6212
    P_6201(6201),
    P_6202(6202),
    P_6203(6203),
    P_6204(6204),
    P_6207(6207),
    P_6208(6208),
    P_6209(6209),
    P_6210(6210),
    P_6211(6211),
    P_6212(6212),
    // endregion

    // region /feature/market 6300-6310
    /** マーケット出品完了。 */
    P_6300(6300),
    /** マーケット出品取り下げ完了。 */
    P_6301(6301),
    /** マーケット購入完了。 */
    P_6302(6302),
    /** オンライン出品者の売上受領通知。 */
    P_6303(6303),
    /** マーケットに出品できないアイテム。 */
    P_6304(6304),
    /** マーケット操作失敗。 */
    P_6305(6305),
    /** 出品枠上限到達。 */
    P_6306(6306),
    /** マーケット購入用 Gold 不足。 */
    P_6307(6307),
    /** マーケット取り下げ時の所持品容量不足。 */
    P_6308(6308),
    /** 自分の出品を購入しようとした。 */
    P_6309(6309),
    /** 出品取消の SQL 確定結果を確認中。 */
    P_6310(6310),
    /** 出品作成の SQL 確定結果を確認中。 */
    P_6311(6311),
    // endregion

    // region /feature/webauth 6400-6409
    P_6400(6400),
    P_6401(6401),
    P_6402(6402),
    P_6403(6403),
    P_6404(6404),
    // endregion

    // region /feature/boss 6500-6539
    P_6500(6500),
    P_6501(6501),
    P_6502(6502),
    P_6504(6504),
    /** 既存のボスまたは他の挑戦と競合しています。{0}: 挑戦中の表示名 */
    P_6505(6505),
    P_6506(6506),
    P_6507(6507),
    P_6508(6508),
    P_6509(6509),
    P_6510(6510),
    /** ボス討伐完了。{0}: ボス表示名、{1}: 討伐時間（秒） */
    P_6511(6511),
    P_6512(6512),
    P_6513(6513),
    P_6514(6514),
    P_6515(6515),
    P_6516(6516),
    P_6517(6517),
    P_6518(6518),
    P_6519(6519),
    P_6520(6520),
    P_6521(6521),
    P_6522(6522),
    P_6523(6523),
    P_6524(6524),
    P_6525(6525),
    P_6526(6526),
    P_6527(6527),
    P_6528(6528),
    P_6529(6529),
    P_6530(6530),
    P_6531(6531),
    P_6532(6532),
    /** ボスインスタンス作成待機 subtitle。{0}: 順番、{1}: 待機人数 */
    P_6534(6534),
    /** 待機ハブへ到着したメンバーから未到着メンバーへのボス挑戦待機通知。{0}: 到着メンバー、{1}: ボス表示名 */
    P_6535(6535),
    /** 管理者向けのボス／ダンジョン挑戦一覧件数。{0}: 件数 */
    P_6536(6536),
    /** 管理者向けのボス／ダンジョン挑戦一覧行。{0}: 挑戦情報 */
    P_6537(6537),
    /** 管理者向け一覧の空表示 */
    P_6538(6538),
    /** 管理者の挑戦リーダー位置テレポート失敗 */
    P_6539(6539),
    // endregion

    // region /feature/quest 6600-6619
    P_6600(6600),
    P_6601(6601),
    P_6602(6602),
    P_6603(6603),
    P_6604(6604),
    P_6605(6605),
    P_6606(6606),
    P_6607(6607),
    P_6608(6608),
    P_6609(6609),
    // endregion

    // region common display labels 6700-6719
    /** 処理が完了するまでお待ちください。 */
    P_6700(6700),
    P_6701(6701),
    P_6702(6702),
    P_6703(6703),
    P_6704(6704),
    P_6705(6705),
    P_6706(6706),
    P_6707(6707),
    /** パーティー一覧のリーダー表示。 */
    P_6708(6708),
    /** 挑戦待機列からの離脱確認見出し。 */
    P_6709(6709),
    /** 挑戦待機列からの離脱確認説明。 */
    P_6710(6710),
    /** 挑戦待機列から離脱する確認操作。 */
    P_6711(6711),
    /** 挑戦待機列に残る確認操作。 */
    P_6712(6712),
    /** 挑戦受付後にパーティーメンバーを待つサイドバー状態。 */
    P_6713(6713),
    /** 挑戦受付後に作成順を待つサイドバー状態。 */
    P_6714(6714),
    // endregion

    // region /feature/currency 6800-6819
    /** 最上位通貨を所持していないため、カレンシー画面から両替所を開けない。 */
    P_6800(6800),
    // endregion

    // region /feature/player/service/PlayerMessageService.java 5940-5949
    P_5940(5940),
    P_5941(5941),
    P_5942(5942),
    P_5943(5943),
    P_5944(5944),
    P_5945(5945),
    P_5946(5946),
    P_5947(5947),
    /** クラス正式名・クラスレベルタグ付きのタブプレイヤー名。 */
    P_5948(5948),
    /** 所持品内に共有対象のアイテムが見つからない。 */
    P_5949(5949),
    // endregion

    // region /feature/teleporter 5950-5969
    P_5950(5950),
    P_5951(5951),
    P_5952(5952),
    P_5953(5953),
    P_5954(5954),
    P_5955(5955),
    P_5956(5956),
    P_5957(5957),
    P_5958(5958),
    P_5959(5959),
    P_5960(5960),
    P_5961(5961),
    P_5962(5962),
    P_5963(5963),
    P_5964(5964),
    // endregion

    // region /feature/party/recruitment 5970-5989
    /** パーティーの募集内容を更新した。 */
    P_5970(5970),
    /** 募集内容が未設定または空。 */
    P_5971(5971),
    /** 募集内容が最大文字数を超えている。{0}: 最大文字数 */
    P_5972(5972),
    /** パーティー掲示板への掲載を開始した。 */
    P_5973(5973),
    /** パーティー掲示板への掲載を停止した。 */
    P_5974(5974),
    /** 掲示板参加を承認制へ切り替えた。 */
    P_5975(5975),
    /** 掲示板参加を承認不要へ切り替えた。 */
    P_5976(5976),
    /** 掲示板の募集が終了または参加不能になっている。 */
    P_5977(5977),
    /** 掲示板から参加申請を送った。{0}: パーティー名 */
    P_5978(5978),
    /** リーダーが参加申請を受信した。{0}: 申請者名 */
    P_5979(5979),
    /** 指定プレイヤーの参加申請が見つからない。{0}: プレイヤー名 */
    P_5980(5980),
    /** リーダーが参加申請を承認した。{0}: プレイヤー名 */
    P_5981(5981),
    /** 掲示板経由でパーティーへ参加した。{0}: パーティー名 */
    P_5982(5982),
    /** 同じパーティーへ参加申請済み。 */
    P_5983(5983),
    /** パーティーが満員になり掲示板掲載を自動終了した。 */
    P_5985(5985),
    // endregion

    // region /feature/particle/command/ParticleCommand.java 6900-6909
    /** パーティクル表示完了。{0}: ID {1}: 量 {2}: 秒数（0は単発） */
    P_6900(6900),
    /** パーティクル ID 不正。{0}: 入力値 */
    P_6901(6901),
    /** パーティクル量不正。{0}: 最大値 */
    P_6902(6902),
    /** パーティクル量または時間が数値でない。{0}: 項目名 */
    P_6903(6903),
    /** パーティクル表示時間不正。{0}: 最大値 */
    P_6904(6904),
    /** パーティクル表示サービス未初期化。 */
    P_6905(6905),
    // endregion

    // region /feature/hud/command/AdminMessageCommand.java 6910-6919
    /** 管理者メッセージの表示開始。{0}: 秒数 */
    P_6910(6910),
    /** 管理者メッセージの表示時間不正。{0}: 最大秒数 */
    P_6911(6911),
    /** 管理者メッセージの表示時間が数値でない。 */
    P_6912(6912),
    // endregion

    // region /feature/player/command/CreativeFlySpeedCommand.java 6920-6929
    /** クリエイティブ飛行速度設定の成功通知。{0}: 対象プレイヤー名、{1}: パーセンテージ */
    P_6920(6920),
    /** クリエイティブ飛行速度パーセンテージの範囲不正。{0}: 最小、{1}: 最大 */
    P_6921(6921),
    /** クリエイティブ飛行速度パーセンテージの数値形式不正。 */
    P_6922(6922),
    // endregion

    // region /feature/sound/command/SoundCommand.java 6930-6939
    /** サウンド再生完了。{0}: サウンド名、{1}: 音量、{2}: ピッチ */
    P_6930(6930),
    /** サウンド名不正。{0}: 入力値 */
    P_6931(6931),
    /** サウンドの数値引数不正。{0}: 項目名 */
    P_6932(6932),
    /** サウンドの数値引数の範囲不正。{0}: 項目名、{1}: 最小値、{2}: 最大値 */
    P_6933(6933),
    // endregion

    // region /feature/discord 6990-6999
    /** Discordから受信した全体チャットの識別タグ。 */
    P_6990(6990),
    // endregion

    // region /feature/dungeon 7000-7019
    /** ダンジョン機能を利用できません。 */
    P_7000(7000),
    /** 予約領域。 */
    P_7001(7001),
    /** ダンジョン ID が見つかりません。{0}: ID */
    P_7002(7002),
    /** 参加人数が範囲外です。{0}: 最小 {1}: 最大 {2}: 現在 */
    P_7004(7004),
    /** 参加者の誰かが別のダンジョンまたはボスへ挑戦中です。{0}: 挑戦中の表示名 */
    P_7005(7005),
    /** 予約領域。 */
    P_7006(7006),
    /** 予約領域。 */
    P_7007(7007),
    /** ダンジョン準備受付。{0}: 表示名 */
    P_7008(7008),
    /** ダンジョン準備失敗。{0}: 表示名 */
    P_7009(7009),
    /** 部屋戦闘開始。{0}: 進行番号 */
    P_7010(7010),
    /** 部屋クリア。{0}: 進行番号 */
    P_7011(7011),
    /** ボス討伐・踏破。{0}: 表示名 */
    P_7012(7012),
    /** ダンジョン終了。{0}: 表示名 */
    P_7013(7013),
    /** 離脱対象セッションなし。 */
    P_7014(7014),
    /** ダンジョン離脱完了。 */
    P_7015(7015),
    /** ダンジョン内ブロック操作禁止。 */
    P_7016(7016),
    /** ダンジョン離脱転送失敗。 */
    P_7017(7017),
    /** ダンジョン受付範囲外 */
    P_7018(7018),
    /** ダンジョン待機ハブ利用不可 */
    P_7019(7019),
    /** 開始カウントダウン。{0}: 残り秒 */
    P_7020(7020),
    /** カウントダウン対象名。{0}: ダンジョン名 */
    P_7021(7021),
    /** 挑戦開始 title。{0}: ダンジョン名 */
    P_7022(7022),
    /** ダンジョン開始 subtitle。 */
    P_7023(7023),
    /** 挑戦開始後のパーティー招待・参加拒否。 */
    P_7024(7024),
    /** 受付から同じインスタンスへ再参加。 */
    P_7025(7025),
    /** 死亡後の復帰待機。{0}: 秒 {1}: 死亡数 {2}: 許容数 */
    P_7026(7026),
    /** 死亡許容回数超過。{0}: 死亡数 {1}: 許容数 */
    P_7027(7027),
    /** ダンジョン中止はリーダーのみ。 */
    P_7028(7028),
    /** 中止対象セッションなし。 */
    P_7029(7029),
    /** ダンジョン中止完了。 */
    P_7030(7030),
    /** 報酬を収容できない。 */
    P_7031(7031),
    /** 報酬対象外または受取期間終了。 */
    P_7032(7032),
    /** クリア報酬チェスト表示。{0}: ダンジョン名 */
    P_7033(7033),
    /** 帰還ゲート表示。 */
    P_7034(7034),
    /** 受付ゲート表示。{0}: ダンジョン名、{1}: 推奨レベル */
    P_7035(7035),
    /** ダンジョン中止 controller 表示。 */
    P_7036(7036),
    /** ダンジョン中止 GUI title。 */
    P_7037(7037),
    /** ダンジョン中止 GUI item。 */
    P_7038(7038),
    /** ダンジョン中止 GUI lore。 */
    P_7039(7039),
    /** ダンジョン報酬 GUI title。{0}: ダンジョン名 */
    P_7040(7040),
    /** ダンジョン報酬 GUI 前ページ。 */
    P_7041(7041),
    /** ダンジョン報酬 GUI 次ページ。 */
    P_7042(7042),
    /** ダンジョン報酬 GUI 未登録報酬。 */
    P_7043(7043),
    /** ダンジョン報酬 GUI 受取不可。 */
    P_7044(7044),
    /** ダンジョン報酬 GUI 数量。{0}: 数量 */
    P_7045(7045),
    /** ダンジョン報酬 GUI 設定確率。{0}: percent */
    P_7046(7046),
    /** ダンジョン報酬 GUI 受取操作。 */
    P_7047(7047),
    /** カルトグラフ現在地図 title。{0}: ダンジョン表示名 */
    P_7048(7048),
    /** カルトグラフ未発見部屋。 */
    P_7049(7049),
    /** カルトグラフ探索可能部屋。 */
    P_7050(7050),
    /** ダンジョン攻略中部屋。 */
    P_7051(7051),
    /** ダンジョン攻略済み部屋。 */
    P_7052(7052),
    /** カルトグラフ現在地。 */
    P_7053(7053),
    /** ダンジョン入口部屋。 */
    P_7054(7054),
    /** ダンジョン通常部屋。 */
    P_7055(7055),
    /** ダンジョン最深部。 */
    P_7056(7056),
    /** カルトグラフ部屋様式。{0}: 表示名 */
    P_7057(7057),
    /** 標準区画表示名。 */
    P_7058(7058),
    /** 支柱回廊表示名。 */
    P_7059(7059),
    /** 崩落区画表示名。 */
    P_7060(7060),
    /** 鉱脈の間表示名。 */
    P_7061(7061),
    /** カルトグラフ耐久不足。 */
    P_7062(7062),
    /** カルトグラフ装備個体解決失敗。 */
    P_7063(7063),
    /** カルトグラフ新規地図登録。 */
    P_7064(7064),
    /** 踏破記録ロード中。 */
    P_7065(7065),
    /** 踏破記録ロード失敗。 */
    P_7066(7066),
    /** 踏破記録なし。 */
    P_7067(7067),
    /** 踏破記録一覧 title。 */
    P_7068(7068),
    /** 踏破回数。{0}: 回数 */
    P_7069(7069),
    /** 最終踏破日時。{0}: 日時 */
    P_7070(7070),
    /** 踏破記録詳細操作。 */
    P_7071(7071),
    /** 獲得可能アイテム title。{0}: ダンジョン表示名 */
    P_7072(7072),
    /** 獲得可能アイテムなし。 */
    P_7073(7073),
    /** 獲得可能数量。{0}: 数量または範囲 */
    P_7074(7074),
    /** 失効済み現在地図。 */
    P_7075(7075),
    /** ダンジョン内の部屋状態表示見出し。{0}: 区画番号、{1}: 部屋役割 */
    P_7076(7076),
    /** ダンジョン開始部屋の安全地帯表示。 */
    P_7077(7077),
    /** ダンジョン部屋の未開放表示。 */
    P_7078(7078),
    /** ダンジョン部屋の進入可能表示。 */
    P_7079(7079),
    /** カルトグラフの向き表示（北）。 */
    P_7080(7080),
    /** カルトグラフの向き表示（北東）。 */
    P_7081(7081),
    /** カルトグラフの向き表示（東）。 */
    P_7082(7082),
    /** カルトグラフの向き表示（南東）。 */
    P_7083(7083),
    /** カルトグラフの向き表示（南）。 */
    P_7084(7084),
    /** カルトグラフの向き表示（南西）。 */
    P_7085(7085),
    /** カルトグラフの向き表示（西）。 */
    P_7086(7086),
    /** カルトグラフの向き表示（北西）。 */
    P_7087(7087),
    /** カルトグラフの通路表示。 */
    P_7088(7088),
    /** 攻略済み部屋への移動案内。 */
    P_7089(7089),
    /** カルトグラフの移動対象失効。 */
    P_7090(7090),
    /** カルトグラフの部屋移動失敗。 */
    P_7091(7091),
    /** カルトグラフの緊急未踏査区画転送ボタン。 */
    P_7092(7092),
    /** ダンジョンインスタンス作成待機 subtitle。{0}: 順番、{1}: 待機人数 */
    P_7093(7093),
    /** 待機ハブへ到着したメンバーから未到着メンバーへのダンジョン挑戦待機通知。{0}: 到着メンバー、{1}: ダンジョン表示名 */
    P_7094(7094),
    /** カルトグラフの緊急転送クールダウン。 */
    P_7095(7095),
    /** カルトグラフの緊急転送の注意書き。 */
    P_7096(7096),
    /** カルトグラフの緊急転送の管理者監視注意。 */
    P_7097(7097),
    /** カルトグラフの緊急転送先区画選択 title。 */
    P_7098(7098),
    /** ダンジョンのプレイヤー向け区間番号。{0}: 番号 */
    P_7099(7099),
    // endregion

    // region /feature/player/command/UuidCommand.java 7100-7109
    /** プレイヤー UUID 表示。{0}: プレイヤー名、{1}: UUID */
    P_7100(7100),
    /** 緊急未踏査区画転送の参加者通知。{0}: 操作プレイヤー名 */
    P_7101(7101),
    /** 緊急未踏査区画転送失敗。 */
    P_7102(7102),
    /** ダンジョン挑戦時間超過。{0}: ダンジョン表示名 */
    P_7103(7103),
    /** カルトグラフ耐久切れ。 */
    P_7104(7104),
    /** ダンジョンクリア title。{0}: ダンジョン名 */
    P_7105(7105),
    /** ダンジョンクリア subtitle。 */
    P_7106(7106),
    /** Bedrock 接続プレイヤー向けのクリア報酬再表示コマンド subtitle。 */
    P_7107(7107),
    /** ダンジョン報酬 GUI 一括受取。 */
    P_7108(7108),
    /** ダンジョン報酬 GUI 一括受取条件。 */
    P_7109(7109),
    // endregion

    // region /feature/whitelist 7110-7119
    /** whitelist を有効化した通知。 */
    P_7110(7110),
    /** whitelist を無効化した通知。 */
    P_7111(7111),
    /** whitelist による接続拒否。 */
    P_7112(7112),
    /** メンテナンス開始による既存接続の遮断。 */
    P_7113(7113),
    /** whitelist コマンドの不正な引数。 */
    P_7114(7114),
    /** whitelist 対象のDBユーザーが見つからない。{0}: プレイヤー名 */
    P_7115(7115),
    /** whitelist ユーザー追加完了。{0}: プレイヤー名 */
    P_7116(7116),
    /** whitelist ユーザー削除完了。{0}: プレイヤー名 */
    P_7117(7117),
    /** whitelist ユーザー更新処理中。 */
    P_7118(7118),
    /**
     * Network管理中のwhitelist変更拒否
     */
    P_7119(7119),
    // endregion

    // region /feature/player/afk 7120-7129
    /** AFK状態タイトル。 */
    P_7120(7120),
    /** AFK状態のプレイヤー名接頭辞。 */
    P_7121(7121),
    /** クラスマスター未ロード時のTab表示名。 */
    P_7122(7122),
    // endregion

    // region /feature/skill/executor/active/swordsman 7130-7139
    /** エクゼプトスタンプのスニーク急降下案内。 */
    P_7130(7130),
    // endregion

    // region /feature/player/event/PlayerCapacityEventHandler.java 7140-7149
    /** 接続可能人数に達したため接続を拒否するメッセージ。 */
    P_7140(7140),
    // endregion

    // region /feature/network 7150-7159
    /** Proxyネットワーク連携が利用できない。 */
    P_7150(7150),
    /** サーバー移動がすでに処理中。 */
    P_7151(7151),
    /** プレイヤーデータのロード完了前でサーバー移動できない。 */
    P_7152(7152),
    /** 保存失敗によりサーバー移動を中止する。 */
    P_7153(7153),
    /** Proxy接続確認失敗によりサーバー移動を中止する。 */
    P_7154(7154),
    // endregion

    // region /feature/world/command/AdminWorldTeleportItemCommand.java 7160-7169
    /** 管理者用ワールドテレポートアイテムを付与した通知。 */
    P_7160(7160),
    // endregion

    // region /feature/item/castdisk 7170-7179
    /** スキルキャストディスクの指定武器がホットバーにない。 */
    P_7170(7170),
    // endregion

    // region /feature/rebirth 7180-7189
    /** 転生開始完了。{0}: 消費Gold、{1}: 転生前レベル */
    P_7180(7180),
    /** レベル1のため転生不可。 */
    P_7181(7181),
    /** すでに転生中。 */
    P_7182(7182),
    /** 転生Gold不足。{0}: 必要Gold */
    P_7183(7183),
    /** 転生開始失敗。 */
    P_7184(7184),
    /** 転生の有償終了完了。{0}: 消費アストラルド、{1}: 復帰レベル */
    P_7185(7185),
    /** 転生中でないため終了不可。 */
    P_7186(7186),
    /** 転生終了用アストラルド不足。{0}: 必要量 */
    P_7187(7187),
    /** 転生操作処理中。 */
    P_7188(7188),
    /** 転生終了失敗。 */
    P_7189(7189),
    /** 転生中プレイヤーレベルアップ。{0}: レベル、{1}: EXP */
    P_7190(7190),
    /** 管理者EXP付与。{0}: EXP、{1}: プレイヤー名 */
    P_7191(7191),
    // endregion

    // region /feature/skill/executor/active/paladin 7200-7209
    /** ガード不足によりスキルを発動できない。 */
    P_7200(7200),
    /** ガーディアンプロテクトの対象が視線上にいない。 */
    P_7201(7201),
    /** ガーディアンプロテクトを受けた通知。{0}: 発動者名、{1}: 持続秒数 */
    P_7202(7202),
    // endregion

    /** 保存不能を検出した時刻とUUIDの報告案内。 */
    P_7210(7210),
    /** プレイヤー状態の再読み込み失敗。 */
    P_7211(7211),
    // region /feature/account/command 7400-7419
    /** 管理者によるアカウント作成成功。{0}: 対象名、{1}: スロット、{2}: アカウント表示名 */
    P_7400(7400),
    /** 対象プレイヤーへのアカウント作成通知。{0}: スロット、{1}: アカウント表示名 */
    P_7401(7401),
    /** アカウント作成サービス未初期化。 */
    P_7402(7402),
    /** 作成対象ユーザー未検出。 */
    P_7403(7403),
    /** アカウントUUID。{0}: 名称、{1}: スロット、{2}: UUID */
    P_7420(7420),
    /** 複製先上書き確認。{0}: ユーザー、{1}: スロット、{2}: 名称、{3}: 確認トークン */
    P_7421(7421),
    /** 上書き確認トークン無効。 */
    P_7422(7422),
    /** 複製成功。{0}: 複製元、{1}: 対象ユーザー、{2}: スロット、{3}: 作成名、{4}: 作成UUID */
    P_7423(7423),
    /** 複製失敗または結果不明。 */
    P_7424(7424),
    /** 自分自身への複製禁止。 */
    P_7425(7425),
    /** 複製元または対象ユーザー未登録。 */
    P_7426(7426),
    /** 複製先スロット範囲外。 */
    P_7427(7427),
    /** 使用中セッションを保存可能な状態にできない。 */
    P_7428(7428),
    /** 複製後の再読込失敗。 */
    P_7429(7429),
    /** 有効なアカウントセッションの存在により複製拒否。 */
    P_7430(7430),
    /** 複製先の変更により再確認が必要。 */
    P_7431(7431),
    /** 複製先ユーザーの全スロットが使用中。 */
    P_7432(7432),
    // endregion
    // region /feature/donation 7300-7309
    /** 寄付申請の結果。{0}: API から取得した装飾なしの通知文 */
    P_7300(7300),
    /** 未受取報酬付きメールの削除を拒否。 */
    P_7301(7301),
    P_7302(7302),
    // endregion
    /** スキルツリー定義と保存済み状態の世代不一致により、データを保持して再参加を保留する。 */
    P_9050(9050),
    /** アカウント特典通知。 */
    P_7600(7600),
    /** アカウント特典通知。 */
    P_7601(7601),
    /** アカウント特典通知。 */
    P_7602(7602),
    /** アカウント特典通知。 */
    P_7603(7603),
    /** アカウント特典通知。 */
    P_7604(7604),
    /** アカウント特典通知。 */
    P_7605(7605),
    /** アカウント特典通知。 */
    P_7606(7606),
    /** アカウント特典通知。 */
    P_7607(7607),
    /** アカウント特典通知。 */
    P_7608(7608),
    ;

    private final String id;

    /**
     * コンストラクタ。Enum名から接頭辞（最初のアンダースコアまで）を抽出し、番号と結合します。
     *
     * @param number メッセージIDの番号部分
     */
    PlayerMsgId(int number) {
        String name = this.name();
        String prefix = name.substring(0, name.indexOf('_') + 1);
        this.id = prefix + number;
    }

    /**
     * IDを取得します。
     *
     * @return メッセージID
     */
    public String getId() {
        return id;
    }
}
