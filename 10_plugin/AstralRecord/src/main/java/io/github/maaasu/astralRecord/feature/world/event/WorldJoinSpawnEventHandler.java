...
public class WorldJoinSpawnEventHandler extends AbstractEventHandler {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        plugin.getServer().getScheduler().runTask(plugin, () -> runSafely(() -> {
            var world = resolveJoinSpawnWorld();
            if (world == null) {
    ...
    private org.bukkit.World resolveJoinSpawnWorld() {
        var world = Bukkit.getWorld(joinSpawnLocation.world());
        if (world != null) {
            return world;
        }

        WorldMasterData data = worldService.getById(joinSpawnLocation.world());
        return data == null ? null : worldService.resolveLoadedWorld(data);
    }
}
