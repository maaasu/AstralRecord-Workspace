...
本書は、2026-07-29 時点で Plugin に実装されている戦闘・経験値計算を、バランス調整で参照しやすい形に整理したものである。

- 記載する数値・式・適用順は、現行実装の把握用である。
- 防御係数やシールド仕様などを「目標バランスとして採用済み」と定める文書ではない。
...
| 状態異常 DoT | `DamageService.applyConditionDamage` | なし | なし | なし | 通常属性は不使用 | あり |
...
`SUPER_CRITICAL_DAMAGE = 30` は「30%加算」ではなく `× 0.30` である。設定値によっては、超星会心成立時の主撃が成立前より小さくなる。
...
被弾者が管理対象の場合だけ、会心後の合計ダメージへ防御を1回適用する。共通防御と種別防御には、それぞれ別の貫通率を使う。
...
防御は属性分割前の合計ダメージへ1回だけ適用する。属性ごとに防御を繰り返し差し引かない。
...
最大シールドと現在シールドがともに正の場合、その一撃はシールドだけへ反映する。
...
シールドがない場合は、シールド判定前ダメージを HP へ反映する。

- プレイヤーと通常 Mob の HP 下限は0。
- `MobInstance.nonLethal = true` の Mob は HP 1 で止まり、死亡・drop 処理へ進まない。
- ライフスティールは、シールドダメージや残 HP を超える過剰ダメージを含めず、実際に減った HP を基準とする。
- ライフスティール回復量は `実HPダメージ × max(0, LIFE_STEAL) / 100`。回復処理ではさらに `HEALING_INCREASE` を適用する。
...
計算結果は `212.16` である。詳細ログでは小数第1位まで表示するため `HP212.2` となる。相手に有効なシールドがある場合は、この値を HP へ入れず、`212.16` をシールド専用式へ渡す。
...
| 初期方向 | ランダムな水平方位、上向き45度 |
| 初期直進 | 10～20 tick、0.1 block/tick |
| 追尾速度 | 0.4 block/tick |
| 索敵範囲 | 同一 world・半径24 block |
| 寿命 | 生成から100 tick |

初期直進後は、攻撃可能な最寄りの Mob を毎 tick 選び直す。発生元プレイヤーの死亡・ログアウト、表示 entity の消滅、Mob への接触、寿命到達で終了する。
...
状態異常耐性には負の値を設定できる。`POISON` はシールド判定より前に、対象 HP を1未満にしない値へ制限する。その結果が正で、有効なシールドがある場合はシールドを消費する。
...
次の項目は現行実装の説明ではなく、今後のバランス設計で決める事項である。

- 防御係数 `0.5` を目標バランスとして採用するか。
- シールドが一撃の HP ダメージを完全に遮断し、余剰ダメージを通さない仕様を採用するか。
- true damage または「会心・防御を無視する固定ダメージ」を追加するか。
