...
| `user_id` | `UNIQUEIDENTIFIER` |  | ✓ |  | 対象ユーザー UUID |
...
| `is_deleted` | `BIT` |  | ✓ | `0` | プレイヤー単位の一覧非表示フラグ |
...
| `FK_player_mail_state_user` | `user_id` | FK | `dbo.user(uuid)` 参照 |
| `UX_player_mail_state_user_mail` | `user_id`, `mail_id` | UNIQUE | 1 ユーザー 1 メール 1 状態に制限 |
| `IX_player_mail_state_user_id` | `user_id` | NONCLUSTERED | ユーザー単位の一覧取得 |
