...
| `permission`     | `INT`              |    |    ○    |  `0`   | 権限レベル（`0`: 一般 / `99`: 管理者）。許可値を追加する場合は API の権限定義、Plugin enum、DB CHECK 制約を同期更新する |
...
| 制約名 | カラム | 条件 | 説明 |
|:--|:--|:--|:--|
| `CK_user_permission` | `permission` | `IN (0, 99)` | プレイヤー、管理者だけを許可する |
...
旧 permission=5 は20260926_account_benefits移行で0へ変換する。有償VIP期間はアカウント単位の dbo.account_benefits に独立保存し、自動移行しない。
