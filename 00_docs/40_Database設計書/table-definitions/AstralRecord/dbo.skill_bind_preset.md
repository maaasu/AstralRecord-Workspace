...
| `active_skill_slots_json` | NVARCHAR(MAX) | × | `[]` | アクティブスロット 1〜12。リング表示は1〜8。JSON 配列、空きは null |
...
| `passive_skill_slots_json` | NVARCHAR(MAX) | × | `[]` | パッシブ系スキル 12 スロット。JSON 配列、空きは null |
| `is_unlocked` | BIT | × | 0 | 解放済み判定。1 から 3 は API で常時解放扱い |
| `is_selected` | BIT | × | 0 | 現在選択中のプリセット。アカウントごとに未削除行の1件だけ1 |
...
- 一意制約: `UX_skill_bind_preset_account_selected (account_id)`、未削除かつ`is_selected=1`の行のみ
...
- 既存配列は値と位置を保持し、12枠に不足する末尾を API が null で補完する。
- 既存DBには `migrations/20260921_skill_bind_capacity.sql` を適用してプリセット番号制約を拡張する。JSONデータの一括書き換えは不要。
- 各文字列にはスキルマスタ ID ではなく `learned_skill_id` を保存する。通常攻撃予約値だけは例外とする。
- 個体ID導入前の旧プリセットに生のskill IDが残っている場合、スキルID改名時は該当文字列も同じデータ移行で更新し、次回読込で現行の `learned_skill_id` へ正規化する。
