-- 実ロード世代・処理権限・変更要求の導入。既存状態の世代はNULLのまま保持する。
SET XACT_ABORT ON;
BEGIN TRANSACTION;
IF COL_LENGTH(N'dbo.account_skilltree_state', N'definition_generation_id') IS NULL
    ALTER TABLE [dbo].[account_skilltree_state] ADD [definition_generation_id] NVARCHAR(64) NULL;

IF OBJECT_ID(N'dbo.skilltree_definition_generation', N'U') IS NULL
BEGIN
...
IF OBJECT_ID(N'dbo.skilltree_server_runtime', N'U') IS NULL
BEGIN
...
IF OBJECT_ID(N'dbo.skilltree_account_session', N'U') IS NULL
BEGIN
CREATE TABLE [dbo].[skilltree_account_session] (
    [account_session_id] UNIQUEIDENTIFIER NOT NULL,
    [account_id] UNIQUEIDENTIFIER NOT NULL,
    [server_id] NVARCHAR(64) NOT NULL,
    [server_session_id] UNIQUEIDENTIFIER NOT NULL,
    [definition_generation_id] NVARCHAR(64) NOT NULL,
    [lease_token_hash] NVARCHAR(64) NOT NULL,
    [created_at_utc] DATETIME2(3) NOT NULL,
    [expires_at_utc] DATETIME2(3) NOT NULL,
    [closed] BIT NOT NULL,
    [view_sequence] BIGINT NOT NULL,
    CONSTRAINT [PK_skilltree_account_session] PRIMARY KEY CLUSTERED ([account_session_id]),
    CONSTRAINT [FK_skilltree_account_session_account] FOREIGN KEY ([account_id]) REFERENCES [dbo].[account] ([uuid]) ON DELETE CASCADE,
    CONSTRAINT [FK_skilltree_account_session_generation] FOREIGN KEY ([definition_generation_id]) REFERENCES [dbo].[skilltree_definition_generation] ([definition_generation_id])
);
END;

IF OBJECT_ID(N'dbo.skilltree_server_player_view', N'U') IS NULL
BEGIN
CREATE TABLE [dbo].[skilltree_server_player_view] (
    [server_id] NVARCHAR(64) NOT NULL,
    [account_id] UNIQUEIDENTIFIER NOT NULL,
    [server_session_id] UNIQUEIDENTIFIER NOT NULL,
    [account_session_id] UNIQUEIDENTIFIER NULL,
    [definition_generation_id] NVARCHAR(64) NOT NULL,
...
IF OBJECT_ID(N'dbo.skilltree_operation', N'U') IS NULL
BEGIN
CREATE TABLE [dbo].[skilltree_operation] (
    ...
    [claimed_account_session_id] UNIQUEIDENTIFIER NULL,
...
IF OBJECT_ID(N'dbo.skilltree_migration_operation', N'U') IS NULL
BEGIN
CREATE TABLE [dbo].[skilltree_migration_operation] (
    ...
    CONSTRAINT [PK_skilltree_migration_operation] PRIMARY KEY CLUSTERED ([operation_id]),
    CONSTRAINT [FK_skilltree_migration_operation_account] FOREIGN KEY ([account_id]) REFERENCES [dbo].[account] ([uuid]),
    CONSTRAINT [FK_skilltree_migration_operation_generation] FOREIGN KEY ([to_generation_id]) REFERENCES [dbo].[skilltree_definition_generation] ([definition_generation_id]),
    CONSTRAINT [CK_skilltree_migration_operation_result] CHECK (ISJSON([result_json]) = 1)
);
END;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [object_id] = OBJECT_ID(N'dbo.skilltree_account_session') AND [name] = N'UX_skilltree_account_session_active')
    CREATE UNIQUE NONCLUSTERED INDEX [UX_skilltree_account_session_active] ON [dbo].[skilltree_account_session] ([account_id]) WHERE [closed] = 0;
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [object_id] = OBJECT_ID(N'dbo.skilltree_server_player_view') AND [name] = N'IX_skilltree_server_player_view_seen')
    CREATE NONCLUSTERED INDEX [IX_skilltree_server_player_view_seen] ON [dbo].[skilltree_server_player_view] ([last_seen_utc]);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [object_id] = OBJECT_ID(N'dbo.skilltree_operation') AND [name] = N'IX_skilltree_operation_account_status')
    CREATE NONCLUSTERED INDEX [IX_skilltree_operation_account_status] ON [dbo].[skilltree_operation] ([account_id], [status]);
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE [object_id] = OBJECT_ID(N'dbo.skilltree_operation') AND [name] = N'IX_skilltree_operation_server_status')
