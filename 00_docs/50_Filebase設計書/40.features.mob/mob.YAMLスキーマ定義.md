...
各カテゴリ固有のフィールドは、それぞれのスキーマ定義を参照してください。

| カテゴリ    | 固有スキーマ                   | 説明                                |
|:--------|:-------------------------|:----------------------------------|
| `ENEMY` | `enemy.YAMLスキーマ定義.md` | 通常の敵Mob。フィールドやダンジョンに配置            |
| `BOSS`  | `boss.YAMLスキーマ定義.md`   | ボスMob。基本動作はENEMYと同様。固有ギミックはプラグイン側 |
| `NPC`   | `npc.YAMLスキーマ定義.md`     | 非戦闘Mob。会話・ショップなどのインタラクション用        |
...
StatusType の一覧は プラグイン設計書 [[07_1.00-モデル定義]] を参照してください。
...
共通フィールドのみの最小構成例です。カテゴリ固有フィールドを含む完全な例は各スキーマ定義を参照してください。
