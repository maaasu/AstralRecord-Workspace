...
> **StatusType について**: `status` フィールドに使用できるステータス名の一覧は プラグイン設計書 [[07_1.00-モデル定義]] を参照してください。
...
| `equipment[].setId`                                       | String        | ×  | -     | このアイテムが属するセット効果ID（例: `iron_warrior_set`）。セット効果定義は `set_effect.YAMLスキーマ定義.md` を参照。                                                  |
...
セット効果を参照する場合は `set:` prefix を使用します（aliases: `st`）。セット効果スキーマの詳細は `set_effect.YAMLスキーマ定義.md` を参照。
