...
`skill_gem_raw_exchange` は、初級から神級までの上位原石1個を直下位原石3個へ変換する7段階を扱います。逆方向の交換は持ちません。`skill_gem_exchange` は、無印原石1個から各自動生成スキルジェム1個への交換を扱います。既存例外の `administrator_shield_recharge` は、上級原石1個からジェム1個への交換を維持します。初級原石からジェムへの交換は持ちません。
