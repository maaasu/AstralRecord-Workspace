...
- 採集オブジェクトの `level` 以下の `GATHERING_LEVEL` を持つツールを、同じ progression までに入手可能にします。
- 配置場所や景観は個別 world 側で扱います。
- `drops.items[].luckAffected` は、特別な設計理由がない限り `true` とし、幸運補正を反映します。`false` にする場合は、個別定義または関連設計書へ理由を残します。
- 複数プレイヤーが同じ採集オブジェクトへ同時にアクセスした場合、各プレイヤーの採集速度を個別にHPへ反映し、破壊時は貢献者ごとにMob報酬と同じ独立抽選を行います。
