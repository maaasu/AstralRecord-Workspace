...
`starter` のウィザード領域は、`1535`〜`1654` のstatus node 120個で構成する。全nodeは `pointCost: 1`、`unlockCondition.classId: wizard` とし、skill nodeを置かない。既存メイジの北・東・南・西にある4終端から、それぞれ共通魔導1円環へ入り、エレメンタル2円環とアーケイン2円環へ分岐する。
...
| 方向 | メイジ接続元 | 共通魔導 | エレメンタル | アーケイン |
|:--|:--|:--|:--|:--|
| 北 | `1313` | `1535`〜`1540` | `1559`〜`1570` | `1607`〜`1618` |
| 東 | `1320` | `1541`〜`1546` | `1571`〜`1582` | `1619`〜`1630` |
| 南 | `1326` | `1547`〜`1552` | `1583`〜`1594` | `1631`〜`1642` |
| 西 | `1306` | `1553`〜`1558` | `1595`〜`1606` | `1643`〜`1654` |

各方向は6nodeの共通魔導円環を入口とし、その外側へ6node円環2個のエレメンタル枝と6node円環2個のアーケイン枝を扇状に配置する。4方向で20円環を構成し、各円環は6edgeの閉じた輪、円環間は内向きnodeと外向きnodeを結ぶ短いedgeとする。パラディンと同等の小円環間隔で外周範囲内へ収め、座標重複回避のためX方向へ`-0.1`ずらす。ウィザード領域の座標範囲はX `-30.1`〜`29.9`、Z `-30`〜`30`とする。ウィザード領域内のedge同士は交差・重複させず、異なる職業領域の専用edgeとの交差は許容する。
...
全statusは120CPである。共通を含むShadowまたはSpecterの全取得は72CPとなる。Lv80の79CPでは一方の完成形と他系統の入口まで、Lv100の99CPでは一方の完成形と他系統の途中まで取得できるが、両系統の全取得はできない。既存の接続形状を維持したまま上下反転して0.4倍に縮小し、パラディン外周範囲内へ配置する。座標重複回避のためZ方向へ`+0.1`ずらし、座標範囲はX `-18.7`〜`22.1`、Z `-31`〜`-1.6`、相対Yは0とする。
...
ソードマスター領域は既存の接続形状を維持したままX/Z軸を入れ替えて0.7倍に縮小し、パラディン外周範囲内へ配置する。座標重複回避のためZ方向へ`-0.1`ずらし、座標範囲はX `6.7`〜`30.1`、Z `-29.9`〜`31.9`とする。

## シャープシューター専門枝

`starter` のシャープシューター領域は、`2120`〜`2239` のstatus node 120個で構成する。全nodeは `pointCost: 1`、`unlockCondition.classId: sharpshooter` とし、skill効果は持たない。ハンターの間接攻撃notable `1266` から入口へ接続する。
...
共通射撃は6node円環3個、SniperとRapidはそれぞれ6node円環6個で構成し、各円環の手前へ2nodeの星路を置く。共通射撃の最外周からSniperを東側、Rapidを西側へ分岐させる。各円環は入口から二経路へ分かれ、各経路の通常nodeを通って同じnotableへ合流する。

全statusは120CPである。共通射撃を含むSniperまたはRapidの全取得は72CPとなる。Lv80の79CPでは一方の完成形と他系統7CP、Lv100の99CPでは一方の完成形と他系統27CPまで取得できるが、両系統の全取得はできない。将来skill nodeを追加する場合はstatus nodeへ効果を混載せず、関連する円環から独立leafとして分岐する。

シャープシューター領域は既存の接続形状を維持したまま0.6倍に縮小し、パラディン外周範囲内へ配置する。座標重複回避のためX方向へ`+0.1`ずらし、座標範囲はX `-33.8`〜`28.8`、Z `-24.9`〜`-9.7`とする。


`starter` は全867node・940edgeで、全nodeの相対Yは0に統一する。全体の座標範囲はX `-33.9`〜`31.3`、Z `-32.8`〜`31.9`である。二次職の120 status node領域はパラディンの外周範囲を共有し、ウィザードはX `-0.1`、ソードマスターはZ `-0.1`、ファントムアーチャーはZ `+0.1`、シャープシューターはX `+0.1`のオフセットを使い、同一座標を避ける。自己接続、無向edge重複を持たず、rootから全nodeへ到達できることを必須とする。
...
`starter` 構造は、全職共通のPP基礎領域、冒険者CP基礎幹、ソードマン・ハンター・メイジのCP基礎幹・専門枝・skill解放、パラディンの共通防御・Holy・Guardian領域および各固有skill解放、ウィザードの共通魔導・エレメンタル・アーケイン領域、ファントムアーチャーの共通・Shadow・Specter領域、ソードマスターの共通剣技・剣聖・剣舞領域、シャープシューターの共通射撃・Sniper・Rapid領域を定義します。現行は867nodeで構成し、職業条件が異なるnodeも含めて同一座標へ重ねません。
...
| シャープシューター共通射撃 / Sniper / Rapid | 120 | シャープシューターCP 120（24 / 48 / 48） | `classId: sharpshooter` |
...
- ウィザードはメイジの4方向終端 `1306` / `1313` / `1320` / `1326` から、各方向30nodeの円環群へ入ります。各方向は共通魔導1円環からエレメンタル2円環とアーケイン2円環へ分岐し、全体を中心から外周へ広がる20個の6node円環として配置します。エレメンタルは火・氷・雷のダメージと貫通を同率で伸ばし、アーケインは最大MP、魔法攻撃力、知力、スキルダメージ、魔法防御貫通、詠唱短縮を伸ばします。120nodeはすべてstatus nodeとし、skill使用許可を混載しません。
...
- シャープシューターはハンターの間接攻撃notable `1266` を共通入口 `2120`へ接続し、共通射撃24nodeの先から東のSniper 48nodeと西のRapid 48nodeへ分岐します。固有skillとskill nodeは追加しません。雷属性statusは将来の雷属性攻撃実装を前提としつつ、同じ円環へ現行攻撃にも作用する汎用補正を併記します。
