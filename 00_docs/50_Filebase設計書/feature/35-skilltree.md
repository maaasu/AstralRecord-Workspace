...
ウィザードのstatus node内部は4個の連結成分と116edgeで構成し、各成分を親領域の異なる終端から接続する。内部edgeは閉路を持たない木構造とし、円形に見える部分もedgeを閉じず、象限内を大きく回り込む開いた円弧として配置する。能力上の共通魔導、エレメンタル、アーケインという区分は維持するが、単一の共通幹から2枝へ分岐する形には集約しない。
...
- 同一職業領域内、共通領域edge、領域境界edgeに交差・重複がないこと。異なる職業領域の専用edge同士の交差は許容します。
...
5. 編集前に、status node数、主円弧数、補助円弧数、親領域からの入口、内部成分数、閉路の有無、座標変換、占有範囲を配置計画として確定します。
6. 構造JSONへ保存した後、JSON Schemaと意味検証に加え、四象限のnode数、X/Z幅、内部edge長、同一職業領域内と共通領域edgeの交差、開いた円弧形状、対象職業だけを表示したプレビューを確認します。
