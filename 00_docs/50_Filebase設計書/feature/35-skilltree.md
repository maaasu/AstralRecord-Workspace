...
`starter` は全867node・905edgeで、全nodeの相対Yは0に統一する。全体の座標範囲はX `-33.9`〜`31.3`、Z `-32.8`〜`31.9`である。二次職の120 status node領域は同じ外周範囲を共有し、ウィザードはZ `+0.1`、ファントムアーチャーはZ `+0.1`、シャープシューターはX `+0.1`のオフセットを使い、同一座標を避ける。自己接続、無向edge重複を持たず、rootから全nodeへ到達できることを必須とする。
