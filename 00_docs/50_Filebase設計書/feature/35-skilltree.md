...
ウィザード領域は既存の接続形状を維持したままX/Z軸を入れ替えて0.5倍に縮小し、パラディン外周範囲内へ配置する。座標重複回避のためX方向へ`-0.1`ずらし、座標範囲はX `-7.1`〜`30.1`、Z `-0.8`〜`31.7`とする。ウィザード領域内のedge同士は交差・重複させず、異なる職業領域の専用edgeとの交差は許容する。
...
`starter` は全867node・920edgeで、全nodeの相対Yは0に統一する。全体の座標範囲はX `-33.9`〜`31.3`、Z `-32.8`〜`31.9`である。二次職の120 status node領域はパラディンの外周範囲を共有し、ウィザードはX `-0.1`、ソードマスターはZ `-0.1`、ファントムアーチャーはZ `+0.1`、シャープシューターはX `+0.1`のオフセットを使い、同一座標を避ける。自己接続、無向edge重複を持たず、rootから全nodeへ到達できることを必須とする。
