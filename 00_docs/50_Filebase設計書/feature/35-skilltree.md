...
CPの代表Lv80経路は共通幹9CP、断罪枝39CP、聖柩枝31CPの計79CPで、Lv100でも99CPのため黒聖域を含む全132CPは取得できない。全386nodeの相対Yは0に統一する。既存226nodeの座標と既存node間edgeは維持し、パラディンの共通幹は既存東側外周から東へ延ばす。断罪・聖柩・黒聖域は、それぞれ北・東・南の外周に小さな対称ダイヤを連ね、active／optional passiveの全skill nodeは対応するダイヤの外向きleafとする。追加PPは既存西側外周から対称ダイヤと分岐として展開する。追加部分の171edgeを含む全419edgeとノード定義は構造JSONで管理する。
