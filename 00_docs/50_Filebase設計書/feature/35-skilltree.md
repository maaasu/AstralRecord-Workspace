...
`starter` 構造は、全職共通・冒険者・一次職の基礎ステータス幹と、最初の実skill解放を定義します。status 効果を持つノードは148個、将来のskill予約枠は15個、実際の `skill` effectは1件、合計164個です。

| 地域 | status node | 消費 | 解放条件 |
|:--|--:|:--|:--|
| root | 1 | PP 0 | なし |
| root直近の無料PP | 8 | PP 0 | なし |
| 有料PP | 29 | PP 29 | Player Lv.2～30 |
...
- root は中心 `(0, 0, 0)` に置き、上下左右の各方向へ3ブロック間隔で2個ずつ、合計8個の無料PP status nodeを置きます。無料ノードにも基礎status効果を持たせます。
- 有料PPは十字の4方向へ北8、東7、南7、西7の順で置き、すべて1PPとします。必要Player Lv.は北が `2, 6, 10, 14, 18, 22, 26, 30`、東が `3, 7, 11, 15, 19, 23, 27`、南が `4, 8, 12, 16, 20, 24, 28`、西が `5, 9, 13, 17, 21, 25, 29` です。Lv.2～30を一度ずつ使うため、Player Lv.30で得る29PPと一致します。
...
- 一次職は各方向のanchorから、他職の非表示nodeを中継せずに4本の基礎幹を直接伸ばします。各職27nodeはすべて1CPで、Class Lv.28の27CPで全取得できます。今回の27nodeはほぼ全員が取得する基礎配分であり、プレイヤーごとの差を作るビルド用status nodeと実skill nodeは未実装です。

### 将来skill予約枠

将来のskill用に、status nodeとは別に16個の予約枠を置きます。内訳はPP地域4、冒険者地域3、ソードマン・ハンター・メイジ地域が各3です。
...
予約枠 `1202` はソードマンの `swordsman_blade_counter` 使用許可nodeへ置換済みです。ソードマンCP 1、`classId: swordsman` とし、既存構造上のleaf座標とedgeを維持します。
