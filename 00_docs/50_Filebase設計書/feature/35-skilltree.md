...
`starter` のウィザード領域は、`1535`〜`1654` のstatus node 120個で構成する。全nodeは `pointCost: 1`、`unlockCondition.classId: wizard` とし、skill nodeを置かない。既存メイジの終端 `1313` から共通魔導へ入り、その最外周notable `1558` でエレメンタルとアーケインへ分岐する。
...
| 領域 | 単位 | 導入経路 | 6node円環 |
|:--|--:|:--|:--|
| 共通魔導 | 1 | `1535`〜`1536` | `1537`〜`1542` |
| 共通魔導 | 2 | `1543`〜`1544` | `1545`〜`1550` |
| 共通魔導 | 3 | `1551`〜`1552` | `1553`〜`1558` |
| エレメンタル | 1 | `1559`〜`1560` | `1571`〜`1576` |
| エレメンタル | 2 | `1561`〜`1562` | `1577`〜`1582` |
| エレメンタル | 3 | `1563`〜`1564` | `1583`〜`1588` |
| エレメンタル | 4 | `1565`〜`1566` | `1589`〜`1594` |
| エレメンタル | 5 | `1567`〜`1568` | `1595`〜`1600` |
| エレメンタル | 6 | `1569`〜`1570` | `1601`〜`1606` |
| アーケイン | 1 | `1607`〜`1608` | `1619`〜`1624` |
| アーケイン | 2 | `1609`〜`1610` | `1625`〜`1630` |
| アーケイン | 3 | `1611`〜`1612` | `1631`〜`1636` |
| アーケイン | 4 | `1613`〜`1614` | `1637`〜`1642` |
| アーケイン | 5 | `1615`〜`1616` | `1643`〜`1648` |
| アーケイン | 6 | `1617`〜`1618` | `1649`〜`1654` |

共通魔導は3個、エレメンタルとアーケインはそれぞれ6個の標準円環単位で構成する。各単位は2nodeの短い導入経路から6node円環へ入り、円環入口から左右2経路へ分かれてnotableで再合流する。共通魔導を中央の幹として外周へ伸ばし、`1558`からエレメンタルを西側、アーケインを東側へ分け、両枝を左右対称の緩やかな弧として広げる。

親領域との境界edgeは `1313`→`1535` の1本とし、ウィザード内部は134edgeで接続する。全nodeへX方向`-0.1`のオフセットを一律適用し、個別nodeだけをずらさない。相対Yは0、座標範囲はX `-16.5`〜`27.9`、Z `-33.6`〜`-16.2`とする。ウィザード領域内と共通領域edgeとの交差・重複を作らず、異なる職業領域の専用edgeとの交差は許容する。
...
職業地域の共有は、すべての職業で配置構造そのものを一致させる意味ではありません。各職業は、役割に合わせてノード座標、接続、分岐位置、枝の長さを別に設計できます。ただし、共通領域から2つの専門領域へ分かれる上位職の新規配置は、個別設計で別の構造を明記しない限り、後述の「上位職の標準円環配置」に従います。
...
この節の追加後に、共通領域と2つの専門領域で構成する上位職を新規作成する場合、または既存の職業領域を全面再設計する場合は、個別設計で例外を明記しない限り、次の配置構造を使用します。この規則は特定の既存職業、node ID、表示名、能力、保存済み座標を参照元とせず、後からそれらが変更されても連動して変化しない独立した配置契約とします。

この節の追加時点で構造JSONと個別の職業節に定義済みの配置には遡及適用しません。既存配置では各職業節を正本として維持し、座標重複やedge交差を解消する局所調整だけを理由に標準構造へ移行しません。既存配置を全面再設計すると明示した場合に限り、この節を新しい正本として適用します。

- status nodeは共通領域24個、専門領域48個ずつの合計120個を標準とします。pointCostや能力配分は職業設計で決め、配置構造からは決めません。
- 配置の基本単位は、2nodeの短い導入経路と、その先の6node円環を合わせた8nodeとします。6node円環は5個の通常nodeと1個のnotableで構成し、入口から左右2経路へ分かれてnotableで再合流する閉じた輪とします。
- 共通領域は基本単位3個を中心から外周へ直列に接続します。親領域の終端から共通領域の最初の導入経路へは、1本の領域境界edgeで接続します。
- 共通領域の最外周notableから2つの専門領域へ分岐します。各専門領域は基本単位6個を直列に接続し、一方を基準軸の片側、もう一方を反対側へ配置します。
- 2つの専門領域は、円環数、導入経路数、隣接間隔、中心からの距離帯、外周までの到達距離を揃えます。能力、名称、アイコン、notableの効果は異なってよいものの、一方だけを長くする、円環を密集させる、片側だけを直線状にするなど、役割と無関係な形状差を作りません。
- 共通領域を中央の幹として読み取れ、2つの専門領域が左右へ分かれた後、それぞれ外周方向へ緩やかな弧を描く全体シルエットとします。円環の中心と導入経路を一定方向へ機械的に一直線配置しません。
- 各円環は同じ職業領域内でおおむね同じ直径とし、円環内の隣接node間隔と導入経路のnode間隔を揃えます。局所的な座標重複を避けるための調整でも、正六角形に近い輪郭、左右2経路、notableでの再合流を崩しません。
- 配置全体には平行移動、90度単位の回転、軸反転、X/Z軸の入れ替え、X/Z共通の等方拡大縮小を適用できます。変換後の配置済みnodeは構造原点からX/Zともに`-34`〜`34`の範囲へ収め、採用した変換と占有範囲を配置計画へ記録します。この範囲を外れる場合は個別設計の例外として扱います。円環ごとまたは片側の専門領域だけに異なる倍率を適用する非等方変形は行いません。
- 他職業と同じ地域を共有する場合は、全nodeへ同一の0.1刻みオフセットを加えて座標重複を避けます。オフセットは職業領域全体へ一律に適用し、個別nodeだけをずらして円環や左右対称性を崩しません。
- skill nodeを追加する場合はstatus nodeへ効果を混載せず、能力上関連する円環の通常nodeから独立leafとして分岐します。skill nodeを一か所へ集約したり、専門領域の最終notableを必須のskill列で延長したりしません。
- 補助自動配置は下書きにだけ使用でき、最終配置の生成には使用しません。最終座標は、この節の基本単位、分岐、弧状シルエット、左右の均衡を確認して構造JSONへ明示保存します。

この節を適用する新規作成または全面再設計で標準から外れる場合は、構造JSONを編集する前に、相違するnode数、入口数、基本単位、分岐数、分岐方向、座標変換、採用理由を対象の職業節へ記載します。「職業ごとに異なる配置を許容する」という一般方針だけを根拠に、標準構造から暗黙に逸脱しません。

作成後はJSON Schemaと既存の意味検証に加え、次を設計上の受け入れ条件として確認します。

- 共通領域と各専門領域のnode数、基本単位数、入口、分岐元、最終nodeが設計どおりであること。
- 同一職業領域内のedgeに交差・重複がなく、共通領域edgeとも交差・重複しないこと。異なる職業領域の専用edge同士の交差は許容します。
- 2つの専門領域の円環数、距離帯、外周到達距離、全体の占有範囲に意図しない偏りがないこと。
- 円環が閉じた輪として読め、短い導入経路、左右2経路、notableでの再合流が視覚上も崩れていないこと。
- 対象職業だけを表示したプレビューで、共通領域から2つの専門領域へ分かれる進行と、中心から外周へ広がるシルエットを確認できること。
...
- ウィザードはメイジ終端 `1313` から24nodeの共通魔導へ入り、最外周notable `1558` で西側のエレメンタル48nodeと東側のアーケイン48nodeへ分岐します。共通魔導は標準円環単位3個、各専門領域は6個で構成し、左右対称の弧として外周へ広げます。エレメンタルは火・氷・雷のダメージと貫通を同率で伸ばし、アーケインは最大MP、魔法攻撃力、知力、スキルダメージ、魔法防御貫通、詠唱短縮を伸ばします。120nodeはすべてstatus nodeとし、skill使用許可を混載しません。
...
4. 職業別ノードは、対応する職業地域に置きます。共通領域から2つの専門領域へ分かれる上位職は、個別設計に明記した例外がなければ「上位職の標準円環配置」を使用します。
5. 編集前に、領域ごとのnode数、基本単位数、親領域からの入口、専門領域の分岐元、配置方向、座標変換、占有範囲を配置計画として確定します。
6. 構造JSONへ保存した後、JSON Schemaと意味検証に加え、同一職業領域内と共通領域edgeの交差、円環形状、左右の均衡、対象職業だけを表示したプレビューを確認します。
