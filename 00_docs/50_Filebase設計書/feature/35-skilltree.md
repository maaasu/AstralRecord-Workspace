...
CPの代表Lv80経路は共通幹9CP、断罪枝39CP、聖柩枝31CPの計79CPで、Lv100でも99CPのため黒聖域を含む全132CPは取得できない。全386nodeの相対Yは0に統一する。中央の無料PP領域を広げ、パラディンの共通幹から断罪・聖柩・黒聖域を北東・東・南東へ扇状に配置し、追加PPは西側に展開する。既存の職業枝は接続する無料PPノードを基準に四方へ移し、各枝内の円環や間隔を維持する。入口を付け替えると既存プレイヤーの解放済み集合が分断されるため、root直結を含む全408edgeとノード定義は変更しない。
