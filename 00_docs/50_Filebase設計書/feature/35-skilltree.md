...
`starter` にはnode `1362`〜`1521` を追加する。`1362`〜`1487` はパラディンCP 126node、`1505`〜`1510` は各枝のoptional passive skill解放6node、`1488`〜`1504` はclass条件のないPP 17node、`1511`〜`1521` はplayerLv51〜55の低コストPP分岐11nodeである。既存有料PP 32と合わせ、汎用PPは60個の消費先を持つため、playerLv50の49PP・全職先行時のplayerLv51/50PPでも全取得できない選択を残す。CPはLv80で79、Lv100で99しか得られないため、3枝を同時に完走できない。

| 枝 | node | skill | CP設計 |
|:--|:--|:--|:--|
| 共通幹 | 1362〜1370 | なし | 9CP。耐久・Shield・支援力を得る代わりに攻撃系をSCALARで一部下げる。 |
| 断罪 | 1371〜1409、1505〜1506 | アナテマ、ラストジャッジメント、passive 2種 | 幹39CPとoptional passive 2CP。攻撃・STR・skill damage・貫通を上げ、防御またはShieldを下げる。 |
| 聖柩 | 1410〜1448、1507〜1508 | ヴェスパー、セラフィム、passive 2種 | 幹39CPとoptional passive 2CP。SUPPORT_POWER、MP、Shield再充填を上げ、通常攻撃系を下げる。 |
| 黒聖域 | 1449〜1487、1509〜1510 | レクイエム、ブラックサンクタム、passive 2種 | 幹39CPとoptional passive 2CP。Shield・物魔防・HPを上げ、攻撃、機動、MPまたはskill damageを下げる。 |
| 汎用PP | 1488〜1521 | なし | playerLv10〜55で順に解放する全class共通の1PP成長選択。既存PP32と合算して60PPの消費先を持つ。 |

CPの代表Lv80経路は共通幹9CP、断罪枝39CP、聖柩枝31CPの計79CPで、Lv100でも99CPのため黒聖域を含む全132CPは取得できない。全386nodeの相対Yは0に統一する。既存226nodeの座標と全419edgeは維持し、パラディンの共通幹は既存東側外周から東へ延ばす。断罪・黒聖域は北・南へ進んでから西へ回り込み、既存ツリーを囲む外周に対称な四角形・ダイヤの環を連ねる。聖柩は東側へ対称ダイヤを連ね、active／optional passiveの全skill nodeは対応する環の外向きleafとする。追加PPの導入部は西側の既存円環の間を抜け、メイジの円環を横切らずに西の対称ダイヤと分岐へ接続する。追加nodeに関係するedgeは、全職業表示でも他edgeとの交差・重複や無関係なnodeの通過を避ける。ノード定義・消費量・解放条件は配置変更で変えない。
...
`starter` 構造は、全職共通のPP基礎領域、冒険者CP基礎幹、ソードマン・ハンター・メイジ・パラディンのCP基礎幹・専門枝とskill解放を定義します。現行は386nodeで構成し、職業条件が異なるnodeも含めて同一座標へ重ねません。
...
| パラディン共通幹 | 9 | パラディンCP 9 | `classId: paladin` |
| パラディン断罪・聖柩・黒聖域 | 123 | パラディンCP 123 | `classId: paladin` |
