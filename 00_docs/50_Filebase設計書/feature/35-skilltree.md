...
シャープシューター用には、`2120`〜`2239` のstatus node 120個と、`2240`〜`2242` のskill node 3個を定義する。status nodeは `pointCost: 1`、`unlockCondition.classId: sharpshooter` とする。skill node `2240` は `sharpshooter_inheritance_mastery` の使用許可を `CP 1` で与え、共通射撃node `2120` から独立leafとして接続し、相対座標 `(-14.5, 0, -16.0)` に配置する。`2241` のファイアアローと `2242` のアイスアローは定義のみとし、構造JSONへは配置しない。
...
現行構造に配置済みの全statusは120CPで、配置済みskill node `2240` を含む全取得は121CPとなる。共通射撃を含むSniperまたはRapidの全取得は72CPで、継承の心得を含める場合は73CPとなる。未配置の `2241` / `2242` は取得CP計算へ含めない。Lv80の79CPでは一方の完成形と他系統7CP、Lv100の99CPでは一方の完成形と他系統27CPまで取得できるが、両系統の全取得はできない。skill nodeはstatus nodeへ効果を混載せず、配置時は関連する円弧から独立leafとする。
