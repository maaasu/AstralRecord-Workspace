...
`starter` の追加nodeは `1362`〜`1521` の160個である。`1362`〜`1487` と `1505`〜`1510` はパラディンCP 132node、`1488`〜`1504` と `1511`〜`1521` はclass条件のないPP 28nodeである。ノード定義・能力・消費量・解放条件を維持し、追加部分の座標と接続を4方向へ分散する。パラディンはLv80で79CP、Lv100で99CPのため、全132CPを取得できない。追加PPは既存の有料PP 32PPと合わせて60PPの消費先を持ち、playerLv50の49PP・playerLv51の50PPでも全取得できない選択を残す。

| 方向 | 既存ソードマン接続元 → 入口 | パラディンnode | skill | CP設計 |
|:--|:--|:--|:--|:--|
| 北から北西 | 1120 → 1410 | 1410〜1441、1507 | ヴェスパー、ヴェスパー・リタニー、セラフィム | 33CP。聖柩・支援系。 |
| 東から北東 | 1127 → 1371 | 1371〜1402、1505 | アナテマ、ペニテントエッジ、ラストジャッジメント | 33CP。断罪・攻撃系。 |
| 南から南東 | 1359 → 1449 | 1449〜1480、1509 | レクイエム、マーター・ヴァウ、ブラックサンクタム | 33CP。黒聖域・防御系。 |
| 西から南西 | 1140 → 1362 | 1362〜1370、1403〜1409、1442〜1448、1481〜1487、1506、1508、1510 | ジャッジケイター・スカー、コヴナント・ブルワーク、ブラック・ヴィジル | 33CP。共通耐久と3系統の選択。 |

各方向は接続用status 3node、status 9nodeの円環3個、各円環から独立したskill leaf 1個ずつで構成する。入口側の円環から外側の2円環へ分岐する。単一の9CP共通幹を必須経路とせず、各方向の既存ソードマンnodeから独立して進める。南は既存ソードマン円環の `1359` から伸ばし、`1134` とその後続円環の配置・接続を維持する。パラディンでは祖先職のソードマンが可視であるため、4方向とも既存経路から到達できる。
...
全386nodeの相対Yは0に統一する。既存226nodeの座標と既存node同士の248edgeを維持し、追加部分の176edgeを含め全424edgeとする。追加nodeと他nodeの中心間距離は3ブロック以上とし、既存の基本間隔を下回らない。PP導入部の既存円環間には新しいnodeを置かず、接続線だけを通して既存メイジ等のnodeとの間隔を確保する。追加nodeに関係するedgeは、全職業表示でも他edgeとの交差・重複や無関係なnodeの通過を避ける。
...
| パラディン北・東・南・西の4枝 | 132 | パラディンCP 132（各方向33） | `classId: paladin` |
