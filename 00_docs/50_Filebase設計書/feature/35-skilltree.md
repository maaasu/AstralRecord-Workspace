...
`starter` は全747node・770edgeで、全nodeの相対Yは0に統一する。全体の座標範囲はX `-33.9`〜`31.3`、Z `-32.8`〜`31.9`である。二次職の120 status node領域は同じ外周範囲を共有し、職業条件の違いにかかわらず同一座標を使わない。自己接続、無向edge重複を持たず、rootから全nodeへ到達できることを必須とする。
...
`starter` 構造は、全職共通のPP基礎領域、冒険者CP基礎幹、ソードマン・ハンター・メイジのCP基礎幹・専門枝・skill解放、パラディンの共通防御・Holy・Guardian領域および各固有skill解放、ウィザードの共通魔導・エレメンタル・アーケイン領域、ファントムアーチャーの共通・Shadow・Specter領域、ソードマスターの共通剣技・剣聖・剣舞領域を定義します。現行は747nodeで構成し、職業条件が異なるnodeも含めて同一座標へ重ねません。シャープシューターのnode定義120個は存在しますが、現行の `starter` には含めません。
