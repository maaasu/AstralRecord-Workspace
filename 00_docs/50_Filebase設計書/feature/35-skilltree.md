...
CPの代表Lv80経路は共通幹9CP、断罪枝39CP、聖柩枝31CPの計79CPで、Lv100でも99CPのため黒聖域を含む全126CPは取得できない。nodeは既存平面と交差しないY=18の専用層、PPはY=-18の専用層へ置き、rootから到達できる接続を持つ。
...
`starter` 構造は、全職共通のPP基礎領域、冒険者CP基礎幹、ソードマン・ハンター・メイジ・パラディンのCP基礎幹・専門枝とskill解放を定義します。現行は361nodeで構成し、職業条件が異なるnodeも含めて同一座標へ重ねません。
