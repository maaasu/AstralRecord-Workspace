...
現行定義は冒険者の `adventurer_astral_edge`、`adventurer_blast_arrow`、`adventurer_mana_burst`、`adventurer_smash`、`adventurer_quick_shot`、`adventurer_lightning_bolt` の6件です。専門職の skill は未定・未実装であり、クラスの `usableSkills` は使用許可だけを与え、習得済み個体の作成はジェム消費に限定します。
