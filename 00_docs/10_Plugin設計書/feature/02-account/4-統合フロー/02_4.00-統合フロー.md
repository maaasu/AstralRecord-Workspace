...
1. `PlayerGameModeChangeEvent` を受け、AstralRecord 内部でマークしたゲームモード変更は終了する
2. それ以外ではプレイヤーキャッシュから account mode と permission を取得する
3. permission `99` 以上のプレイヤーが ADVENTURE を選択した場合は `PLAYER`、CREATIVE を選択した場合は `ADMIN` へ更新する
4. アカウントモード更新後、既存のオンライン反映と inventory 反映を実行する
5. SURVIVAL / SPECTATOR は account mode を変更しない。`PLAYER` モードの変更要求は従来どおりキャンセルする
...
4. Mob 撃破・クエスト報酬などでプレイヤーレベルまたはクラスレベルが変化した場合は、プレイヤーレベルからPP、クラス別レベルからCPを算出し、skilltree の派生状態を再計算する。CP/PPそのものは account-skilltree API へ永続化しない
