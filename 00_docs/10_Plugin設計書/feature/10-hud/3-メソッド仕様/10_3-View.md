...
各プレイヤーのリスト名は、ログインデータ反映時・クラス変更時・クラスレベル変更時・クラスマスター再読込時・AFK状態遷移時に `PlayerClassService.updatePlayerListName` が更新し、正式クラス名 `name` と現在クラスレベルを `[クラス名 Lv.レベル]` のタグとしてアカウント名の左へ表示する。AFK中は、その左に赤い`[AFK]`接頭辞を付与する。
