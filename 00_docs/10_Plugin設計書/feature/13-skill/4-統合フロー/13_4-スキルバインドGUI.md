...
シジルは空きslot、許可ID、同一`equipGroupId`なしを満たす場合だけ消費装着する。スキルジェムと非許可シジルは素材選択へ進めず、理由を表示して合成APIを呼ばない。素材選択中は正本を削除せず表示だけを隠し、戻る・close・失敗なら復元する。先行インベントリ同期が失敗しても、素材entryをAPI正本が検証するため、シジル装着mutationはその後に続行して必ず再同期する。成功時の API 正本再同期で削除された素材 entry の後続 entry は通常収納と同じ前詰め規則で詰める。結果クリックが確定操作であり、別の確認ダイアログは出さない。
