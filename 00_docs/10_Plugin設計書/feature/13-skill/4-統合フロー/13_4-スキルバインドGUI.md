...
sequenceDiagram
    ...
    Player->>Shop: スキルジェム商品を選択
    Shop-->>Player: 習得 / Lv.N→N+1 / MAXを表示
    Player->>Shop: 購入確定
    Shop->>Inventory: 支払い・ジェム1個付与・保存
    Inventory-->>Shop: 保存成功
    alt 未習得
        Shop->>API: learn(skillId, 購入entryId)
    else 習得済みかつ最大未満
        Shop->>API: levelUp(learnedSkillId, 購入entryId)
    end
    API->>API: serializable transactionで購入ジェム消費 + 習得状態更新
    API-->>Shop: 習得個体
    Shop-->>Player: インベントリとショップ表示を更新
```

未習得なら新規個体をLv.1で作成し、習得済みならAPIロード順の先頭個体だけを1レベル上げる。最大レベル、習得ロード前、定義不在、購入反映中は購入確定を拒否し、支払いもジェム付与も行わない。最大レベルでは `※ すでにレベルMAXのため購入できません。` を表示する。インベントリ内のジェムクリックから習得・レベルアップする導線は持たない。
...
シジルは空きslot、許可ID、同一`equipGroupId`なしを満たす場合だけ、対応する SIGIL_ATTACH オーブとともに各1個を消費して装着する。SIGIL_DETACH は対応するオーブを1個消費し、指定した装着シジルを1個返却する。スキルジェムと非許可シジルは素材選択へ進めず、理由を表示して合成APIを呼ばない。素材選択中はインベントリ正本を変更せずシジルitem IDだけを保持し、entry IDは保持しない。結果クリック直前にオーブと選択シジルitem IDを通常アイテム共通消費順（BAG優先、後方slot優先、slot未指定は後、同slotは`inventoryEntryId`昇順）でentryへ再解決し、そのentry IDをAPIへ渡す。戻る・close・失敗なら選択状態を破棄する。先行インベントリ同期が失敗しても、オーブentryと素材entryをAPI正本が検証するため、シジルmutationはその後に続行して必ず両entryを再同期する。成功時の API 正本再同期で削除された素材 entry の後続 entry は通常収納と同じ前詰め規則で詰める。結果クリックが確定操作であり、別の確認ダイアログは出さない。
