...
sequenceDiagram
    ...
    Service->>Repo: markRead(accountId, updatedBy, mailId)(async)
    alt API成功
...
- 既読化成功 callback の `rewardReceived=true` では控えめな `GuiSound.ITEM_RECEIVE`、`false` ではダミーアイテムの通常クリックと同じ `GuiSound.SELECT` を再生する。
...
sequenceDiagram
    ...
    Service->>Repo: markRead(accountId, updatedBy, mailId)
    alt 既読化成功
