...
修理と強化を `MenuScreen.EQUIPMENT_PROCESSING` の 54 slot GUI に集約する。画面名は `装備加工（修理・強化）` とし、`MenuInventoryHolder.contentId` は初期表示の `repair` または `enhancement` とする。

| 用途 | GUI slot | 内容 |
|:--|---:|:--|
| ガイド / 修理タブ / 強化タブ / 情報 | `10` / `12` / `14` / `16` | `BOOK` / `ANVIL` / `ENCHANTING_TABLE` / 修理は `SPYGLASS`、強化は `NETHER_STAR`。タブ名の先頭 `▶` で選択中モードを示す |
| 対象装備 | `20` | 両モード共通で下段から一時退避してセットする。対象枠クリックで所持品へ返却する |
| 消費素材 | `21..23` | 強化に使う実アイテムアイコンと必要数 / 所持数。素材マスタを解決できない場合は ID でなく `未登録の素材` と表示し、強化を実行不可にする |
