...
詳細画面は slot `4` にプロフィール、slot `22` に装備、`10` から `16` にリソース、基本能力値、攻撃、防御、属性、状態異常、ユーティリティの全 status category、`30` にクラス、`31` にスキル情報、`32` に有効バフ、`38` にアイテム・送金、`42` に party 招待を配置する。アイテム・送金は対象への送信 GUI を開き、戻る操作でこの情報画面へ戻る。各 status category の lore と詳細 GUIでは合計値が 0 の status を表示せず、カテゴリ内に表示可能な status がない場合は灰色で「ステータス情報はありません」と表示する。装備 icon は自分の詳細画面では編集可能な装備画面へ、他プレイヤーの詳細画面では対象装備を参照する専用画面へ遷移させる。参照専用画面では上段・下段のクリックによる装備脱着、所持品移動、装備状態保存を行わない。各 status category icon をクリックすると、そのカテゴリに属する status だけを1 status 1 itemで表示する詳細 GUIへ遷移する。詳細 item は `StatusSnapshot` のステータス名、説明、現在値、基礎値、合計補正を表示し、項目数に応じてページングする。属性・状態異常の status は種類ごとの表示色を付け、属性・状態異常カテゴリ内ではダメージ・付与確率などの増加系、耐性系、貫通系の順に表示する。

スキル情報 icon の lore は、現在選択中の `SkillBindPreset` から解決したアクティブバインドを表示し、仕切り線の後へパッシブバインドを表示する。クリック後は選択画面を挟まず、`SkillPermissionService` の使用許可スキルと `LearnedSkillService` の習得済み個体を統合した一覧を直接開く。習得済みは個体ごとに全件表示し、同じスキルが使用許可済みの場合も使用許可だけの重複項目は追加しない。各項目には使用許可の有無、習得の有無、現在レベル、マスター定義の icon、`SkillPresentationUtil` による名称・説明・効果 lore、装着中のシジルを表示する。内部IDは表示しない。

統合一覧の最下段には「現在のバインド」との切り替え項目を置く。現在のバインド表示は選択中プリセットを読み取り専用で描画し、slot `9` に左クリック枠、slot `19`～`25` にパッシブ枠、slot `28`～`34` にアクション枠を配置する。パッシブ枠はslot `18` / `26`、アクション枠はslot `27` / `35` の左右移動項目で全12枠を横スクロールできる。各設定済み枠はスキルマネージャーと同じ表示生成処理を使い、使用許可、習得レベル、スキル説明、現在有効な枠かどうか、装着中のシジル名・効果を表示する。

## 5. 画面固有の描画不変条件

- `MenuIconFactory` は共有 icon 定義から呼出ごとに独立した `ItemStack` を生成する。`CURRENCY` は指定の textures を持つ `PLAYER_HEAD` を使用し、表示名・基本説明は共通アイコン定義の既存値を使う。通貨詳細は最大 10 件とし、超過時は ellipsis を加える。
...
- 候補クリック後の完成スナップショット保存・SQL ACK待機中は対象slotへ `CLOCK` を一度だけ表示し、候補、下段、ホットバー、ページ、戻るを含むすべてのクリックを無効化する。固定フレーム演出・反復サウンド・確定後の追加待機は行わない。
...
- SQL ACK完了と同じtickで候補全体を再計算して結果音を鳴らす。オーブが0個なら閉じ、残っていれば一覧を維持する。状態変化は完了時に閉じる。
...
クラフト 2×2 枠の既定配置は、左上から順にプレイヤー情報、スキルマネージャー、カレンシー、装備とする。クラフト結果枠のメニューアイコンは専用の固定 textures を持つ `PLAYER_HEAD` とする。
