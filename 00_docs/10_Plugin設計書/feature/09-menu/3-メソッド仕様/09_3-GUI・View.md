...
- `MenuIconFactory` は共有 icon 定義から呼出ごとに独立した `ItemStack` を生成する。`CURRENCY` は指定の textures を持つ `PLAYER_HEAD`（表示名 `Bag of Seeds`、固定 lore 2 行）を使用し、通貨詳細は最大 10 件とし、超過時は ellipsis を加える。
