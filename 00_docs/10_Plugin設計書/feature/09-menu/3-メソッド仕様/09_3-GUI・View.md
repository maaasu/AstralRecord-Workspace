...
旧装備加工タブ画面は削除する。通常プレイヤーインベントリのオーブをクリックすると、そのオーブで現在実行可能な装備だけを候補一覧に表示する。
...
- 候補クリックから10 tickは `AMETHYST_BLOCK` → `BUDDING_AMETHYST` → `CRYING_OBSIDIAN` → `END_CRYSTAL` → `NETHER_STAR` を各2 tickで切り替え、フレームごとにサウンドを鳴らす。その後10 tickは更新待ちとし、合計20 tickは候補、下段、ホットバー、ページ、戻るを含むすべてのクリックを無効化する。
...
- 待機完了時は候補全体を再計算して更新音を鳴らす。オーブが0個なら閉じ、残っていれば一覧を維持する。状態変化は完了後に閉じる。
