...
- 2026-07-11: `HotbarShortcutGuiHolder` 対象 GUI の閉じる操作はプレイヤーインベントリのホットバー `slot 4` を正本とし、チェスト側に閉じるアイコンや不可視の閉じるクリック領域を重複配置しない。ホットバー共通操作を持たない GUI だけがチェスト側の閉じるボタンを使用する。
