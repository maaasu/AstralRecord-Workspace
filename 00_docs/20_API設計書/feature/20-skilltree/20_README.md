...
- 明示移行はPlugin runtime keyと分離したmigration keyでoffline accountにのみ実行する。operation ID、expected state version、旧node集合baseline、from/to世代、明示remove node ID、APIが保存済み世代snapshotからgraph・保持nodeの条件/コストを照合する。APIは新node付与、CP元付替え、Gold変更を行わず、除去nodeの既存spent返還はPlugin既存規則に委ねる。
...
SQL Serverの`skilltree_account_session`参照は`READCOMMITTEDLOCK`でSQL文内だけ共有ロックを保持する。外側のSerializable transactionへsessionの共有範囲ロックを持ち越すと、別accountのview公開・session更新・heartbeatとの循環待ちになるためである。session変更、旧APIの変更拒否判定、補修、snapshot保存権限の検証では、先に取得するaccount行のUPDLOCK/HOLDLOCKによって同accountの競合を防ぐ。runtime行の読取ロックと外側のtransaction分離レベルは変更せず、保存確定までのboot/世代の検証境界を維持する。closeもaccount、runtime、sessionの順で照合する。表示用の単独読取は従来どおり未commit値を参照しない。
