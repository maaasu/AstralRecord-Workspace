...
- 承認額1円につき`99a00021`（CURRENCY、アストラルド(有償)）1個。承認時はManagementDBだけで累計・承認通知を確定し、後続照合で各アカウントの予約済み累計との差分を配布指示にする。メールはアカウントごとに配信し、配信完了のMailDelivered通知は同一ユーザー・同一累計到達額につき最初の配信時点に1件だけ作る。その時点で準備済みの各メールが同数なら、最初に届いたメールの数量入り通知文を維持し、数量が異なる場合だけ数量を省く。後から別アカウントへ同じ累計分を配信しても通知を増やさず、先に表示した通知文も変更しない。
