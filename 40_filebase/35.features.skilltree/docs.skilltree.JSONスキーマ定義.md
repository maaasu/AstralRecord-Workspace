...
- 配置設計では、共通の端点で接続する場合を除き、同一職業領域に属する edge 同士を x-z 平面上で交差または重複させない。両端nodeが同じ`unlockCondition.classId`を持つ edge をその職業領域とし、両端に`classId`がない edge を共通領域とする。異なる職業領域の専用 edge 同士の交差は許容するが、共通領域 edge は他のいずれの edge とも交差・重複させない。異なる`classId`のnodeを結ぶ入口 edge は領域境界 edge として同一職業領域内の交差禁止判定には含めないが、共通領域 edge との交差・重複は禁止する。
- 円環など輪郭を意図したノード群は、間隔と対称性を揃え、ギザギザした線やいびつな形にならないよう配置する。
