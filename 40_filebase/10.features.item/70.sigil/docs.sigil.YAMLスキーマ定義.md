...
たとえば `70a00001` と `70a00002` はどちらも `cooldown_reduction` を指定するため、同一個体へ同時には装着できません。
...
現行の Plugin ソースでは `70a00003` を `hasSigil` で判定する処理は確認できないため、
...
`40_filebase/10.features.item/40.orb/docs.orb.YAMLスキーマ定義.md` を参照してください。

## YAML 例

### 例1: ステータス補正型シジル

以下は現行の `70a00001` マスタです。`COOLDOWN_REDUCTION` を `+5` 加算し、
...
以下は現行の `70a00003` マスタです。ステータス補正を持たないため、
...
- シジル装着・脱着オーブ: `40_filebase/10.features.item/40.orb/docs.orb.YAMLスキーマ定義.md`
