...
$batchMigrationId = '20260921_skilltree_batch_editor'
...
"@
    ...
    $batchMigration = @($config.migrations | Where-Object { $_.id -eq $batchMigrationId })[0]
    if ($null -eq $batchMigration) { throw 'Batch migration manifest is missing.' }
    $config.preExistingMigrationFileNames = @($config.migrations | Where-Object { $_.id -ne $migrationId -and $_.id -ne $batchMigrationId } | ForEach-Object { $_.fileName })
    $config.migrations = @($migration[0], $batchMigration)
    ...
    Assert-ScalarEqual -Name 'batch migration history count' -Actual ([int](Invoke-DbScalar -Database $databaseName -CommandText "SELECT COUNT(*) FROM dbo.schema_migration WHERE migration_id = N'$batchMigrationId';")) -Expected 1
    ...
    & $sqlcmd -S $SqlInstance -d $databaseName -E -C -b -i (Join-Path $config.migrationsRootPath '20260921_skilltree_batch_editor.sql') 2>&1 | Write-Output
    if ($LASTEXITCODE -ne 0) { throw 'The batch migration SQL is not safe to reapply.' }
